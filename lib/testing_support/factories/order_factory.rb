FactoryBot.define do
  factory :order, class: Aypex::Order do
    user
    bill_address
    completed_at { nil }
    email { user&.email }
    ...
    factory :order_with_totals do
      after(:create) do |order, evaluator|
        create(:line_item, order:, price: evaluator.line_items_price)
    ...
    factory :order_with_line_item_quantity do
      ...
      after(:create) do |order, evaluator|
        create(:line_item, order:, price: evaluator.line_items_price, quantity: evaluator.line_items_quantity)
        order.line_items.reload # to ensure order.line_items is accessible after
      end
    end
    ...
    factory :order_with_line_items do
      ...
      transient do
        ...
        shipment_cost { 100 }
        shipping_method_filter { Aypex::ShippingMethod::DISPLAY_ON_FRONT_END }
      end

      after(:create) do |order, evaluator|
        unless evaluator.without_line_items
          create_list(:line_item, evaluator.line_items_count, order:, price: evaluator.line_items_price)
          order.line_items.reload
        end

        stock_location = order.line_items&.first&.variant&.stock_items&.first&.stock_location || create(:stock_location)
        create(:shipment, order:, cost: evaluator.shipment_cost, stock_location:)
      ...
      factory :completed_order_with_totals do
        ...
        factory :completed_order_with_store_credit_payment do
          after(:create) do |order|
            ...
            create(:store_credit_payment, amount: order.total, order:, source: store_credit, payment_method:)
          end
        end
