module Aypex
  ...
  def self.check_missing_translations
    ...
    used_translations.map { |a| a.split(".") }.each do |translation_keys|
      ...
      translation_keys.each do |key|
        ...
        unless Aypex.missing_translation_messages.include?(error)
          Aypex.missing_translation_messages << error
        end
      end
    end
  end
  ...
  def self.check_unused_translations
    ...
    translation_diff.each do |translation|
      Aypex.unused_translation_messages << "#{translation} (#{I18n.locale})"
    end
  end
  ...
  def self.translations
    @translations ||= I18n.backend.send(:translations)[I18n.locale][:aypex]
  end
end
...
RSpec.configure do |config|
  ...
  if ENV["CHECK_TRANSLATIONS"]
    config.after :suite do
      ...
      if Aypex.missing_translation_messages.any?
        puts "\nThere are missing translations within Aypex:"
        puts Aypex.missing_translation_messages.sort
        exit(1)
      end

      Aypex.check_unused_translations
      if Aypex.unused_translation_messages.any?
        puts "\nThere are unused translations within Aypex:"
        puts Aypex.unused_translation_messages.sort
        exit(1)
      end
    end
  end
