module Aypex
  ...
  #   1) Make a copy of it in your sites local /lib/aypex folder
  ...
  module ProductFilters
    ...
    #
    Aypex::Product.add_search_scope :price_range_any do |*opts|
      conds = opts.map { |o| Aypex::ProductFilters.price_filter[:conds][o] }.reject(&:nil?)
      scope = conds.shift
      conds.each do |new_scope|
        scope = scope.or(new_scope)
      end
      Aypex::Product.joins(master: :default_price).where(scope)
    end

    def self.format_price(amount)
      Aypex::Money.new(amount)
    end
    ...
    Aypex::Product.add_search_scope :brand_any do |*opts|
      conds = opts.map { |o| ProductFilters.brand_filter[:conds][o] }.reject(&:nil?)
      scope = conds.shift
      conds.each do |new_scope|
        scope = scope.or(new_scope)
      end
      Aypex::Product.with_property("brand").where(scope)
    end

    def self.brand_filter
      brand_property = Aypex::Property.find_by(name: "brand")
      brands = brand_property ? Aypex::ProductProperty.where(property_id: brand_property.id).pluck(:value).uniq.map(&:to_s) : []
    ...
    Aypex::Product.add_search_scope :selective_brand_any do |*opts|
