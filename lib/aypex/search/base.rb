    ...
    class Base
        ...
        if search.is_a?(ActionController::Parameters)
          search.each do |name, scope_attribute|
            ...
            base_scope = if base_scope.respond_to?(:search_scopes) && base_scope.search_scopes.include?(scope_name.to_sym)
              ...
              base_scope.merge(Aypex::Product.ransack(scope_name => scope_attribute).result)
            end
          end
        end
        base_scope
      ...
      def get_products_option_values_conditions(base_scope, option_value_ids)
        unless option_value_ids.blank?
          base_scope = base_scope.joins(variants: :option_values).where(aypex_option_values: {id: option_value_ids})
        end
        base_scope
      end
      ...
      def get_price_range(price_param)
        ...
        less_than_string = I18n.t("activerecord.attributes.aypex/product.less_than")
      ...
      def build_option_value_ids(params)
        filter_params = Aypex::OptionType.filterable.map(&:filter_param)
      ...
      def prepare(params)
        ...
        @properties[:per_page] = (per_page > 0) ? per_page : Aypex::Config.products_per_page
