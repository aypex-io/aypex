    ...
    module Order
      ...
      def current_order(options = {})
        ...
        if options[:create_order_if_necessary] && (@current_order.nil? || @current_order.completed?)
          ...
          @current_order.associate_user! try_aypex_current_user if try_aypex_current_user
          @current_order.last_ip_address = ip_address
        end
      ...
      def associate_user
        @order ||= current_order
        if try_aypex_current_user && @order
          @order.associate_user!(try_aypex_current_user) if @order.user.blank? || @order.email.blank?
        end
      end

      def set_current_order
        return unless try_aypex_current_user && current_order

        orders_scope = try_aypex_current_user.orders
          .incomplete
          .where.not(id: current_order.id)
          .where(store_id: current_store.id)

        orders_scope.each do |order|
          current_order.merge!(order, try_aypex_current_user)
        end
      end
      ...
      def last_incomplete_order(includes = {})
        @last_incomplete_order ||= try_aypex_current_user.last_incomplete_aypex_order(current_store, includes: includes)
      end

      def current_order_params
        {currency: current_currency, token: cookies.signed[:token], user_id: try_aypex_current_user.try(:id)}
      end
      ...
      def find_order_by_token_or_user(options = {}, with_adjustments = false)
        ...
        order = last_incomplete_order(includes) if order.nil? && try_aypex_current_user
