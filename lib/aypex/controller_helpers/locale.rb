    ...
    module Locale
      ...
      def current_locale
        @current_locale ||= if user_locale?
          try_aypex_current_user.selected_locale
        elsif params_locale?
          params[:locale]
        elsif config_locale?
          config_locale
      ...
      def user_locale?
        current_store.use_the_user_preferred_locale && try_aypex_current_user && supported_locale?(try_aypex_current_user.selected_locale)
      end
      ...
      def supported_locales_for_all_stores
        @supported_locales_for_all_stores ||= Aypex.available_locales
      end

      def available_locales
        Aypex::Store.available_locales
      end
