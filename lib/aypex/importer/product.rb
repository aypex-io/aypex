  ...
  module Importer
    class Product
      ...
      def initialize(product, product_params, options = {})
        @store = options[:store] || Aypex::Store.default
        @product = product || Aypex::Product.new(product_params)
      ...
      def set_up_options
        options_attrs.each do |name|
          option_type = Aypex::OptionType.where(name: name).first_or_initialize do |option_type|
