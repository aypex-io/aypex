  ...
  module Importer
    class Order
      def self.import(user, params)
        ...
        order = Aypex::Order.create! create_params
        ...
        params.delete(:user_id) unless user.try(:has_aypex_role?, "admin") && params.key?(:user_id)
      ...
      def self.create_shipments_from_params(shipments_hash, order)
        ...
        shipments_hash.each do |s|
          ...
          shipment.stock_location = Aypex::StockLocation.find_by(admin_name: s[:stock_location]) ||
            Aypex::StockLocation.find_by!(name: s[:stock_location])
          ...
          shipping_method = Aypex::ShippingMethod.find_by(name: s[:shipping_method]) ||
            Aypex::ShippingMethod.find_by!(admin_name: s[:shipping_method])
      ...
      def self.create_line_items_from_params(line_items, order)
        ...
        line_items.each do |line_item|
          ...
          variant = Aypex::Variant.find(line_item[:variant_id])
      ...
      def self.create_payments_from_params(payments_hash, order)
        ...
        payments_hash.each do |p|
          ...
          # aypex_wombat serializes payment state as status so imported orders should fall back to status field.
          payment.state = p[:state] || p[:status] || "completed"
          payment.created_at = p[:created_at] if p[:created_at]
          payment.payment_method = Aypex::PaymentMethod.find_by!(name: p[:payment_method])
      ...
      def self.ensure_variant_id_from_params(hash)
        ...
        unless hash[:variant_id].present?
          hash[:variant_id] = Aypex::Variant.active.find_by!(sku: sku).id
        end
        hash
      ...
      def self.ensure_country_id_from_params(address)
        ...
        begin
          ...
          address[:country_id] = Aypex::Country.where(search).first!.id
        rescue => e
      ...
      def self.ensure_state_id_from_params(address)
        ...
        begin
          ...
          if state = Aypex::State.where(search).first
