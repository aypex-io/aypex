namespace :exchanges do
  desc "Takes un-returned exchanged items and creates a new order to charge the customer for not returning them"
  task charge_unreturned_items: :environment do
    unreturned_return_items_scope = Aypex::ReturnItem.awaiting_return.exchange_processed
    unreturned_return_items = unreturned_return_items_scope.joins(:exchange_inventory_units).where([
      "aypex_inventory_units.created_at < :days_ago AND aypex_inventory_units.state = :iu_state",
      days_ago: Aypex::Config.expedited_exchanges_days_window.days.ago, iu_state: "shipped"
    ]).distinct.to_a

    # Determine that a return item has already been deemed un-returned and therefore charged
    ...
    unreturned_return_items.group_by(&:exchange_shipments).each do |shipments, return_items|
      ...
      order = Aypex::Order.create!(order_attributes)
      ...
      return_items.group_by(&:exchange_variant).map do |variant, variant_return_items|
        ...
        line_item = Aypex::LineItem.create!(variant: variant, quantity: variant_return_items.count, order: order)
      ...
      unless order.payments.present?
        ...
        Aypex::Payment.create!(order: order,
