...
namespace :db do
  ...
  desc "Fix orphan line items after upgrading to Aypex 3.1: only needed if you have line items attached to deleted records with Slug (product) and SKU (variant) duplicates of non-deleted records."
  task fix_orphan_line_items: :environment do |_t, _args|
    ...
    if get_input
      ...
      variants_to_fix.each do |variant|
        ...
        Aypex::LineItem.unscoped.where(variant_id: variant.id).update_all(variant_id: dup_variant.id)
      end
  ...
  desc "Migrates taxon icons to aypex assets after upgrading to Aypex 3.4: only needed if you used taxons icons."
  task migrate_taxon_icons: :environment do |_t, _args|
    Aypex::Taxon.where.not(icon_file_name: nil).find_each do |taxon|
  ...
  desc "Migrates taxon icons to taxon images after upgrading to Aypex 3.7: only needed if you used taxons icons."
  task migrate_taxon_icons_to_images: :environment do |_t, _args|
    Aypex::Asset.where(type: "Aypex::TaxonIcon").update_all(type: "Aypex::TaxonImage")
  end

  desc "Ensure all Order associated with Store after upgrading to Aypex 3.7"
  task associate_orders_with_store: :environment do |_t, _args|
    Aypex::Order.where(store_id: nil).update_all(store_id: Aypex::Store.default.id)
  end

  desc "Ensure all Order has currency present after upgrading to Aypex 3.7"
  task ensure_order_currency_presence: :environment do |_t, _args|
    Aypex::Order.where(currency: nil).find_in_batches do |orders|
