...
module Aypex
  class Shipment < Aypex::Base
    include Aypex::NumberGenerator.new(prefix: "H", length: 11)
    ...
    with_options inverse_of: :shipments do
      belongs_to :address, class_name: "Aypex::Address"
    ...
    has_one :selected_shipping_rate, -> { where(selected: true).order(:cost) }, class_name: Aypex::ShippingRate.to_s
    ...
    scope :reverse_chronological, -> { order(Arel.sql("coalesce(aypex_shipments.shipped_at, aypex_shipments.created_at) desc"), id: :desc) }
    ...
    def determine_state(order)
      ...
      (order.paid? || Aypex::Config.auto_capture_on_dispatch) ? "ready" : "pending"
    end
    ...
    def free?
      ...
      adjustments.promotion.any? { |p| p.source.type == "Aypex::Promotion::Actions::FreeShipping" }
    end
