module Aypex
  class ReturnItem < Aypex::Base
    ...
    has_many :exchange_inventory_units, class_name: "Aypex::InventoryUnit",
      foreign_key: :original_return_item_id,
      inverse_of: :original_return_item
    belongs_to :exchange_variant, class_name: "Aypex::Variant"
    belongs_to :preferred_reimbursement_type, class_name: "Aypex::ReimbursementType"
    belongs_to :override_reimbursement_type, class_name: "Aypex::ReimbursementType"
    ...
    scope :exchange_required, -> { eager_load(:exchange_inventory_units).where(aypex_inventory_units: {original_return_item_id: nil}).distinct }
    ...
    def currency
      return_authorization.try(:currency) || Aypex::Store.default.default_currency
    end
    ...
    def stock_item
      ...
      Aypex::StockItem.find_by(variant_id: inventory_unit.variant_id,
    ...
    def set_exchange_pre_tax_amount
      self.pre_tax_amount = BigDecimal("0.0") if exchange_requested?
    end

    def validate_no_other_completed_return_items
      other_return_item = Aypex::ReturnItem.where(inventory_unit_id: inventory_unit_id,
    ...
    def cancel_others
      Aypex::ReturnItem.where(inventory_unit_id: inventory_unit_id).where.not(id: id)
        .not_cancelled.each(&:cancel!)
    end

    def should_restock?
      resellable? && variant.should_track_inventory? && stock_item && Aypex::Config.restock_inventory
    end
  end
end
