module Aypex
  class StoreCredit < Aypex::Base
    ...
    DEFAULT_CREATED_BY_EMAIL = "aypex@example.com".freeze

    belongs_to :user, class_name: "::#{Aypex::Config.user_class}", foreign_key: "user_id"
    belongs_to :category, class_name: "Aypex::StoreCreditCategory"
    belongs_to :created_by, class_name: Aypex::Config.admin_user_class.to_s, foreign_key: "created_by_id"
    belongs_to :credit_type, class_name: "Aypex::StoreCreditType", foreign_key: "type_id"
    belongs_to :store, class_name: "Aypex::Store"
    has_many :store_credit_events, class_name: "Aypex::StoreCreditEvent"
    ...
    scope :order_by_priority, -> { includes(:credit_type).order("aypex_store_credit_types.priority ASC") }
    ...
    class << self
      def default_created_by
        Aypex::Config.user_class.find_by(email: DEFAULT_CREATED_BY_EMAIL)
      end
    end
    ...
    def create_credit_record(amount, action_attributes = {})
      ...
      credit = if Aypex::Config.credit_to_new_allocation
        Aypex::StoreCredit.new(create_credit_record_params(amount))
      else
    ...
    def associate_credit_type
      unless type_id
        ...
        self.credit_type = Aypex::StoreCreditType.find_or_create_by(name: credit_type_name)
      end
    end
  end
end
