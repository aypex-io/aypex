...
module Aypex
  class Product < Aypex::Base
    ...
    has_many :promotions, through: :promotion_rules, class_name: "Aypex::Promotion"

    has_many :possible_promotions, -> { advertised.active }, through: :promotion_rules,
      class_name: "Aypex::Promotion",
      source: :promotion
    ...
    validates :slug, presence: true, uniqueness: {allow_blank: true, case_sensitive: true, scope: aypex_base_uniqueness_scope}
    ...
    # by `position` attribute
    #
    # @return [Aypex::Variant]
    def default_variant
      @default_variant ||= Rails.cache.fetch(default_variant_cache_key) do
        if Aypex::Config.track_inventory_levels && available_variant = variants.detect(&:purchasable?)
          available_variant
        else
    ...
    # values should not be displayed to storefront users. Otherwise it breaks the
    ...
    def property(property_name)
      product_properties.joins(:property).find_by(aypex_properties: {name: property_name}).try(:value)
    end

    def set_property(property_name, property_value, property_presentation = property_name)
      ApplicationRecord.transaction do
        # Works around aypex_i18n #301
    ...
    def digital?
      shipping_category&.name == I18n.t("aypex.seed.shipping.categories.digital")
    end

    private

    def add_associations_from_prototype
      if prototype_id && prototype = Aypex::Prototype.find_by(id: prototype_id)
    ...
    def any_variants_not_track_inventory?
      return true unless Aypex::Config.track_inventory_levels
    ...
    def default_variant_cache_key
      "aypex/default-variant/#{cache_key_with_version}/#{Aypex::Config.track_inventory_levels}"
    end
    ...
    def requires_price?
      Aypex::Config.require_master_price
    end
