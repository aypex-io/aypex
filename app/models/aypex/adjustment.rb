module Aypex
  ...
  class Adjustment < Aypex::Base
    ...
    belongs_to :order, class_name: "Aypex::Order", inverse_of: :all_adjustments
    belongs_to :promotion_action, class_name: "Aypex::PromotionAction", foreign_key: :source_id, optional: true # created only for has_free_shipping?
    ...
    self.competing_promos_source_types = ["Aypex::PromotionAction"]

    scope :not_finalized, -> { where(state: "open") }
    scope :finalized, -> { where(state: "closed") }
    scope :tax, -> { where(source_type: "Aypex::TaxRate") }
    scope :non_tax, -> do
      source_type = arel_table[:source_type]
      where(source_type.not_eq("Aypex::TaxRate").or(source_type.eq(nil)))
    end
    scope :price, -> { where(adjustable_type: "Aypex::LineItem") }
    scope :shipping, -> { where(adjustable_type: "Aypex::Shipment") }
    scope :optional, -> { where(mandatory: false) }
    scope :eligible, -> { where(eligible: true) }
    scope :charge, -> { where("#{quoted_table_name}.amount >= 0") }
    scope :credit, -> { where("#{quoted_table_name}.amount < 0") }
    scope :nonzero, -> { where("#{quoted_table_name}.amount != 0") }
    scope :promotion, -> { where(source_type: "Aypex::PromotionAction") }
    scope :return_authorization, -> { where(source_type: "Aypex::ReturnAuthorization") }
    scope :is_included, -> { where(included: true) }
    scope :additional, -> { where(included: false) }
    scope :competing_promos, -> { where(source_type: competing_promos_source_types) }
    scope :for_complete_order, -> { joins(:order).merge(Aypex::Order.complete) }
    scope :for_incomplete_order, -> { joins(:order).merge(Aypex::Order.incomplete) }
    ...
    def promotion?
      source_type == "Aypex::PromotionAction"
    end
