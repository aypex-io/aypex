...
module Aypex
  class Taxon < Aypex::Base
    ...
    belongs_to :taxonomy, class_name: "Aypex::Taxonomy", inverse_of: :taxons
    ...
    has_many :prototype_taxons, class_name: "Aypex::PrototypeTaxon", dependent: :destroy
    has_many :prototypes, through: :prototype_taxons, class_name: "Aypex::Prototype"

    has_many :promotion_rule_taxons, class_name: "Aypex::PromotionRuleTaxon", dependent: :destroy
    has_many :promotion_rules, through: :promotion_rule_taxons, class_name: "Aypex::PromotionRule"
    ...
    scope :for_store, ->(store) { joins(:taxonomy).where(aypex_taxonomies: {store_id: store.id}) }

    self.whitelisted_ransackable_associations = %w[taxonomy]
    self.whitelisted_ransackable_attributes = %w[name permalink]

    scope :for_stores, ->(stores) { joins(:taxonomy).where(aypex_taxonomies: {store_id: stores.ids}) }
    ...
    def applicable_filters
      ...
      fs << Aypex::ProductFilters.price_filter if Aypex::ProductFilters.respond_to?(:price_filter)
      fs << Aypex::ProductFilters.brand_filter if Aypex::ProductFilters.respond_to?(:brand_filter)
      fs
    end
