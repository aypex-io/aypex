  ...
  class Order < Aypex::Base
    module StoreCredit
      ...
      def could_use_store_credit?
        return false if Aypex::PaymentMethod::StoreCredit.available.empty?
      ...
      def display_total_applicable_store_credit
        Aypex::Money.new(-total_applicable_store_credit, currency: currency)
      end

      def display_total_applied_store_credit
        Aypex::Money.new(-total_applied_store_credit, currency: currency)
      end

      def display_order_total_after_store_credit
        Aypex::Money.new(order_total_after_store_credit, currency: currency)
      end

      def display_total_available_store_credit
        Aypex::Money.new(total_available_store_credit, currency: currency)
      end

      def display_store_credit_remaining_after_capture
        Aypex::Money.new(total_available_store_credit - total_applicable_store_credit, currency: currency)
      end
    end
  end
end
