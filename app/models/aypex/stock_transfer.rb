module Aypex
  class StockTransfer < Aypex::Base
    include Aypex::NumberGenerator.new(prefix: "T")
    ...
    def find_stock_location_with_location_id(location_id)
      stock_movements.joins(:stock_item).where("aypex_stock_items.stock_location_id" => location_id)
    end
  end
end
