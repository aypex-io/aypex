module Aypex
  class Variant < Aypex::Base
    ...
    validates :sku, uniqueness: {conditions: -> { where(deleted_at: nil) }, case_sensitive: false, scope: aypex_base_uniqueness_scope},
    ...
    scope :in_stock, -> { joins(:stock_items).where("#{Aypex::StockItem.table_name}.count_on_hand > ? OR #{Aypex::Variant.table_name}.track_inventory = ?", 0, false) }
    scope :backorderable, -> { joins(:stock_items).where(aypex_stock_items: {backorderable: true}) }
    ...
    scope :not_deleted, -> { where("#{Aypex::Variant.quoted_table_name}.deleted_at IS NULL") }

    scope :for_currency_and_available_price_amount, ->(currency = nil) do
      currency ||= Aypex::Store.default.default_currency
      joins(:prices).where("aypex_prices.currency = ?", currency).where("aypex_prices.amount IS NOT NULL").distinct
    end
    ...
    def tax_category
      @tax_category ||= if self[:tax_category_id].nil?
        ...
        Aypex::TaxCategory.find_by(id: self[:tax_category_id]) || product.tax_category
      end
    end

    def options_text
      @options_text ||= Aypex::Variants::OptionsPresenter.new(self).to_sentence
    end
    ...
    def set_option_value(opt_name, opt_value)
      ...
      option_type = Aypex::OptionType.where(["LOWER(name) = ?", opt_name.downcase.strip]).first_or_initialize do |o|
    ...
    def price_in(currency)
      ...
      Rails.cache.fetch("aypex/prices/#{cache_key_with_version}/price_in/#{currency}") do
    ...
    alias_method :is_backorderable?, :backorderable?
    ...
    def should_track_inventory?
      track_inventory? && Aypex::Config.track_inventory_levels
    end
    ...
    def remove_line_items_from_incomplete_orders
      Aypex::Variants::RemoveFromIncompleteOrdersJob.perform_later(self)
    end

    def quantifier
      Aypex::Stock::Quantifier.new(self)
    end
    ...
    def check_price
      if price.nil? && Aypex::Config.require_master_price
    ...
    def set_cost_currency
      self.cost_currency = Aypex::Store.default.default_currency if cost_currency.blank?
    end
    ...
    def disable_sku_validation?
      Aypex::Config.disable_sku_validation
    end
  end
end
