module Aypex
  class TaxRate < Aypex::Base
    ...
    scope :potential_rates_for_zone,
      ->(zone) do
        where(zone_id: Aypex::Zone.potential_matching_zones(zone).pluck(:id))
      end
    scope :for_default_zone,
      -> { potential_rates_for_zone(Aypex::Zone.default_tax) }
    ...
    # https://github.com/aypex/aypex/issues/4318#issuecomment-34723428
    def self.store_pre_tax_amount(item, rates)
      pre_tax_amount = case item.class.to_s
      when "Aypex::LineItem" then item.discounted_amount
      when "Aypex::Shipment" then item.discounted_cost
      end
    ...
    def self.adjust(order, items)
      ...
      Aypex::Adjustment.where(adjustable: items).tax.destroy_all
