require_dependency "aypex/order/checkout"
require_dependency "aypex/order/currency_updater"
require_dependency "aypex/order/digital"
require_dependency "aypex/order/payments"
require_dependency "aypex/order/store_credit"
require_dependency "aypex/order/emails"
...
module Aypex
  class Order < Aypex::Base
    ...
    include Aypex::Order::Checkout
    include Aypex::Order::CurrencyUpdater
    include Aypex::Order::Digital
    include Aypex::Order::Payments
    include Aypex::Order::StoreCredit
    include Aypex::Order::AddressBook
    ...
    if Aypex::Config.user_class
      belongs_to :user, class_name: "::#{Aypex::Config.user_class}", optional: true
    else
      belongs_to :user, optional: true
    end
    if Aypex::Config.admin_user_class
      belongs_to :created_by, class_name: Aypex::Config.admin_user_class.to_s, optional: true
      belongs_to :approver, class_name: Aypex::Config.admin_user_class.to_s, optional: true
      belongs_to :canceler, class_name: Aypex::Config.admin_user_class.to_s, optional: true
    else
    ...
    belongs_to :bill_address, foreign_key: :bill_address_id, class_name: "Aypex::Address",
      optional: true, dependent: :destroy
    alias_attribute :billing_address, :bill_address

    belongs_to :ship_address, foreign_key: :ship_address_id, class_name: "Aypex::Address",
      optional: true, dependent: :destroy
    ...
    with_options dependent: :destroy do
      has_many :state_changes, as: :stateful, class_name: "Aypex::StateChange"
      has_many :line_items, -> { order(:created_at) }, inverse_of: :order, class_name: "Aypex::LineItem"
      has_many :payments, class_name: "Aypex::Payment"
      has_many :return_authorizations, inverse_of: :order, class_name: "Aypex::ReturnAuthorization"
      has_many :adjustments, -> { order(:created_at) }, as: :adjustable, class_name: "Aypex::Adjustment"
    end
    has_many :reimbursements, inverse_of: :order, class_name: "Aypex::Reimbursement"
    has_many :line_item_adjustments, through: :line_items, source: :adjustments
    has_many :inventory_units, inverse_of: :order, class_name: "Aypex::InventoryUnit"
    has_many :return_items, through: :inventory_units, class_name: "Aypex::ReturnItem"
    has_many :variants, through: :line_items
    has_many :products, through: :variants
    has_many :refunds, through: :payments
    has_many :all_adjustments,
      class_name: "Aypex::Adjustment",
      foreign_key: :order_id,
      dependent: :destroy,
      inverse_of: :order

    has_many :order_promotions, class_name: "Aypex::OrderPromotion"
    has_many :promotions, through: :order_promotions, class_name: "Aypex::Promotion"

    has_many :shipments, class_name: "Aypex::Shipment", dependent: :destroy, inverse_of: :order do
      def states
        pluck(:state).uniq
      end
    end
    ...
    scope :reverse_chronological, -> { order(Arel.sql("aypex_orders.completed_at IS NULL"), completed_at: :desc, created_at: :desc) }
    ...
    def confirmation_required?
      store.checkout_always_include_confirm_step ||
    ...
    def tax_address
      store.tax_using_ship_address ? ship_address : bill_address
    end
    ...
    def merger
      @merger ||= Aypex::OrderMerger.new(self)
    end

    def ensure_store_presence
      self.store ||= Aypex::Store.default
    end
    ...
    def create_tax_charge!
      Aypex::TaxRate.adjust(self, line_items)
      Aypex::TaxRate.adjust(self, shipments) if shipments.any?
    end
    ...
    def available_payment_methods(store = nil)
      if store.present?
        ActiveSupport::Deprecation.warn("The `store` parameter is deprecated and will be removed in Aypex 5. Order is already associated with Store")
      end
    ...
    def can_add_coupon?
      Aypex::Promotion.order_activatable?(self)
    end
    ...
    def create_proposed_shipments
      ...
      StateChange.where(stateful_type: "Aypex::Shipment", stateful_id: shipment_ids).delete_all
      ...
      self.shipments = Aypex::Stock::Coordinator.new(self).shipments
    end

    def apply_free_shipping_promotions
      Aypex::PromotionHandler::FreeShipping.new(self).activate
      shipments.each { |shipment| Aypex::Adjustable::AdjustmentsUpdater.update(shipment) }
    ...
    def apply_unassigned_promotions
      ::Aypex::PromotionHandler::Cart.new(self).activate
    end
    ...
    def has_non_reimbursement_related_refunds?
      refunds.non_reimbursement.exists? ||
        payments.offset_payment.exists? # how old versions of aypex stored refunds
    end

    def collect_admin_payment_methods
    ...
    def validate_payments_attributes(attributes)
      ActiveSupport::Deprecation.warn("`Order#validate_payments_attributes` is deprecated and will be removed in Aypex 5")
    ...
    def cart_promo_total
      all_adjustments.eligible.nonzero.promotion
        .where.not(adjustable_type: "Aypex::Shipment")
        .sum(:amount)
    end
    ...
    def has_free_shipping?
      shipment_adjustments
        ...
        .where(aypex_adjustments: {eligible: true, source_type: "Aypex::PromotionAction"},
          aypex_promotion_actions: {type: "Aypex::Promotion::Actions::FreeShipping"}).exists?
    end
