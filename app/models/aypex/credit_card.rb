module Aypex
  class CreditCard < Aypex::Base
    ...
    scope :not_expired, lambda {
      where("CAST(aypex_credit_cards.year AS DECIMAL) > ?", Time.current.year)
        .or(where("CAST(aypex_credit_cards.year AS DECIMAL) = ?", Time.current.year)
           .where("CAST(aypex_credit_cards.month AS DECIMAL) >= ?", Time.current.month))
    }
    ...
    # More info: https://github.com/aypex/aypex/issues/6209
    #
    ...
    # ActiveMerchant needs first_name/last_name because we pass it a Aypex::CreditCard and it calls those methods on it.
