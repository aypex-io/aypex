module Aypex
  class LineItem < Aypex::Base
    ...
    belongs_to :tax_category, -> { with_deleted }, class_name: "Aypex::TaxCategory"
    ...
    validates_with Aypex::Stock::AvailabilityValidator, if: -> { variant.present? }
    ...
    alias_method :discounted_money, :display_discounted_amount
    ...
    alias_method :total, :final_amount
    alias_method :money, :display_total

    def sufficient_stock?
      Aypex::Stock::Quantifier.new(variant).can_supply? quantity
    end
    ...
    def verify_order_inventory
      Aypex::OrderInventory.new(order, self).verify(target_shipment, is_updated: true)
    end

    def verify_order_inventory_before_destroy
      Aypex::OrderInventory.new(order, self).verify(target_shipment)
    end
    ...
    def recalculate_adjustments
      Aypex::Adjustable::AdjustmentsUpdater.update(self)
    end

    def update_tax_charge
      Aypex::TaxRate.adjust(order, [self])
    end
