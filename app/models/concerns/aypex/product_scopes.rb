  ...
  module ProductScopes
    ...
    included do
      ...
      add_search_scope :in_stock do
        joins(:variants_including_master).merge(Aypex::Variant.in_stock)
      end

      add_search_scope :backorderable do
        joins(:variants_including_master).merge(Aypex::Variant.backorderable)
      end
      ...
      #
      #   SELECT COUNT(DISTINCT "aypex_products"."id") ...
      #
      #   vs.
      #
      #   SELECT COUNT(*) ...
      ...
      add_search_scope :with_option do |option|
        if option.is_a?(OptionType)
          joins(:option_types).where(aypex_option_types: {id: option.id})
        elsif option.is_a?(Integer)
          joins(:option_types).where(aypex_option_types: {id: option})
        elsif OptionType.column_for_attribute("id").type == :uuid
          joins(:option_types).where(aypex_option_types: {name: option}).or(Product.joins(:option_types).where(aypex_option_types: {id: option}))
        else
          joins(:option_types).where(aypex_option_types: {name: option})
        end
      end
      ...
      add_search_scope :with_option_value do |option, value|
        ...
        return Product.group("#{Aypex::Product.table_name}.id").none if option_type_id.blank?

        group("#{Aypex::Product.table_name}.id")
          .joins(variants_including_master: :option_values)
          .where(Aypex::OptionValue.table_name => {name: value, option_type_id: option_type_id})
      end
      ...
      # it does not stack with other scopes :/
      #
      ...
      def self.available(available_on = nil, currency = nil)
        ...
        unless Aypex::Config.show_products_without_price
      ...
      def self.for_user(user = nil)
        if user.try(:has_aypex_role?, "admin")
          with_deleted
        else
