  ...
  module Products
    class Find
      def initialize(scope:, params:, current_currency: nil)
        ...
        if current_currency.present?
          ActiveSupport::Deprecation.warn(<<-DEPRECATION, caller)
            `current_currency` param is deprecated and will be removed in Aypex 5.
      ...
      def by_skus(products)
        ...
        products.joins(:variants_including_master).where(aypex_variants: {sku: skus})
      end
      ...
      def by_option_value_ids(products)
        ...
        product_ids = Aypex::Product
          ...
          .where(aypex_option_values: {id: option_value_ids})
          .group("#{Aypex::Product.table_name}.id, #{Aypex::Variant.table_name}.id")
          .having("COUNT(aypex_option_values.option_type_id) = ?", option_types_count(option_value_ids))
          .distinct
          .ids
      ...
      def ordered(products)
        ...
        when "price-high-to-low"
          products
            .select("#{Product.table_name}.*, #{Aypex::Price.table_name}.amount")
            .reorder("")
