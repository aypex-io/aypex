    ...
    context "when order is given" do
      context "when can be destroyed" do
        ...
        it "voids pending payments" do
          expect_any_instance_of(Aypex::Payment).to receive(:void!).exactly(order.payments.count).times

          subject
        end

        it "cancel not shipped shipments" do
          expect_any_instance_of(Aypex::Shipment).to receive(:cancel!).exactly(order.shipments.count).times
        ...
        it "destroys line_items, shipments and payments" do
          ...
          expect(Aypex::LineItem.where(id: line_item_ids)).to be_empty
          expect(Aypex::Shipment.where(id: shipment_ids)).to be_empty
          expect(Aypex::Payment.where(id: payment_ids)).to be_empty
        end

        context "when empty service is called first" do
          before { Aypex::Cart::Empty.call(order: order) }
