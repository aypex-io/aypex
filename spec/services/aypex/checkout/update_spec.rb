...
describe Aypex::Checkout::Update, type: :service do
  ...
  describe "update address" do
    ...
    let(:permitted_attributes) do
      Aypex::PermittedAttributes.checkout_attributes + [
        bill_address_attributes: Aypex::PermittedAttributes.address_attributes,
        ship_address_attributes: Aypex::PermittedAttributes.address_attributes
      ]
    end
    ...
    context "at cart state" do
      ...
      it "sets order back to address state" do
    ...
    context "at address state" do
      ...
      context "when ship and bill address changed" do
        ...
        it "updates both order addresses" do
          expect(update_service).to be_success
          expect(order.reload.bill_address_id).to eq address.id
          expect(order.reload.ship_address_id).to eq address.id
        end
  ...
  describe "update selected shipping rate" do
    ...
    let(:permitted_attributes) do
      Aypex::PermittedAttributes.checkout_attributes + [
        shipments_attributes: Aypex::PermittedAttributes.shipment_attributes
      ]
    end

    it "sets order back to delivery state" do
