...
describe Aypex::Refund, type: :model do
  ...
  describe "create" do
    ...
    before do
      allow(payment.payment_method)
        .to receive(:credit)
        .with(amount_in_cents, payment.source, payment.transaction_id, originator: an_instance_of(Aypex::Refund))
    ...
    context "transaction id exists on creation" do
      ...
      it "creates a refund record" do
        expect { subject }.to change { Aypex::Refund.count }.by(1)
      end
    ...
    context "processing is successful" do
      ...
      it "return the newly created refund" do
        expect(subject).to be_a(Aypex::Refund)
      end
    ...
    context "processing fails" do
      ...
      it "raises error and not create a refund" do
        expect do
          expect { subject }.to raise_error(Aypex::GatewayError, gateway_response_message)
        end.not_to change { Aypex::Refund.count }
      end
    end
    ...
    context "without payment profiles supported" do
      ...
      it "does not supply the payment source" do
        expect(payment.payment_method)
          .to receive(:credit)
          .with(amount * 100, payment.transaction_id, originator: an_instance_of(Aypex::Refund))
          .and_return(gateway_response)
    ...
    context "with an activemerchant gateway connection error" do
      before do
        ...
        expect(payment.payment_method).to receive(:credit).with(
          ...
          originator: an_instance_of(Aypex::Refund)
    ...
    context "with amount too large" do
      ...
      it "is invalid" do
        expect { subject }.to raise_error { |error|
          ...
          expect(error.record.errors.full_messages).to eq ["Amount #{I18n.t("activerecord.errors.models.aypex/refund.attributes.amount.greater_than_allowed")}"]
        }
      end
    end
  end
  ...
  describe "total_amount_reimbursed_for" do
    subject { Aypex::Refund.total_amount_reimbursed_for(reimbursement) }
    ...
    context "with reimbursements performed" do
      ...
      it "returns the total amount" do
        amount = Aypex::Refund.total_amount_reimbursed_for(reimbursement)
