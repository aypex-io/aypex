...
class CustomPromotionAction < Aypex::PromotionAction
...
describe Aypex::Adjustable::Adjuster::Promotion do
  ...
  let(:order_subject) { Aypex::Adjustable::AdjustmentsUpdater.new(order) }

  context "best promotion is always applied" do
    let(:calculator) { Aypex::Calculator::Promotion::FlatRate.new(amount: 10) }
    let(:source) { Aypex::Promotion::Actions::CreateItemAdjustments.create calculator: calculator }
    ...
    describe "competing promos" do
      before { Aypex::Adjustment.competing_promos_source_types = ["Aypex::PromotionAction", "CustomPromotionAction"] }
    ...
    context "when previously ineligible promotions become available" do
      ...
      promo_sequences.each do |promo_sequence|
        it "picks the best line-item-level promo according to current eligibility" do
          ...
          Aypex::Cart::AddItem.call(order: order, variant: create(:variant, price: 10))
          order.save
