...
describe Aypex::PaymentMethod::StoreCredit do
    ...
    context "with a valid request" do
      ...
      context "with an originator" do
        ...
        it "passes the originator" do
          expect_any_instance_of(Aypex::StoreCredit).to receive(:authorize)
  ...
  describe "#capture" do
    ...
    context "when unable to authorize the amount" do
      ...
      before do
        allow_any_instance_of(Aypex::StoreCredit).to receive_messages(authorize: true)
      end
    ...
    context "with a valid request" do
      ...
      context "with an originator" do
        ...
        it "passes the originator" do
          expect_any_instance_of(Aypex::StoreCredit).to receive(:capture)
  ...
  describe "#void" do
    ...
    context "when the store credit is not voided successfully" do
      before { allow_any_instance_of(Aypex::StoreCredit).to receive(:void).and_return false }
    ...
    context "with an originator" do
      ...
      it "passes the originator" do
        expect_any_instance_of(Aypex::StoreCredit).to receive(:void).with(anything, action_originator: originator)
        subject
      end
    end
  end
  ...
  describe "#purchase" do
    it "declines a purchase if it can't find a pending credit for the correct amount" do
      ...
      store_credit.store_credit_events.create!(action: Aypex::StoreCredit::ELIGIBLE_ACTION,
        amount: amount,
        authorization_code: auth_code)
      store_credit.store_credit_events.create!(action: Aypex::StoreCredit::CAPTURE_ACTION,
  ...
  describe "#credit" do
    ...
    context "when the store credit isn't credited successfully" do
      before { allow(Aypex::StoreCredit).to receive_messages(credit: false) }
    ...
    context "with a valid credit request" do
      before { allow_any_instance_of(Aypex::StoreCredit).to receive_messages(credit: true) }
    ...
    context "with an originator" do
      ...
      it "passes the originator" do
        allow_any_instance_of(Aypex::StoreCredit).to receive(:credit)
          .with(anything, anything, anything, action_originator: originator)
        subject
      end
    end
  ...
  describe "#payment_source_class" do
    ...
    it { expect(store_credit_payment_method.payment_source_class).to eq(Aypex::StoreCredit) }
  end
end
