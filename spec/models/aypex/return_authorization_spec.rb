...
describe Aypex::ReturnAuthorization do
  ...
  let(:return_authorization) do
    Aypex::ReturnAuthorization.new(order: order,
    ...
    context "expedited exchanges are configured" do
      ...
      context "no items to exchange" do
        ...
        it "does not create a reimbursement" do
          expect { subject.save }.not_to change { Aypex::Reimbursement.count }
        end
      end
      ...
      context "items to exchange" do
        ...
        it "performs an exchange reimbursement for the exchange return items" do
          subject.save
          reimbursement = Aypex::Reimbursement.last
        ...
        context "the reimbursement fails" do
          before do
            allow_any_instance_of(Aypex::Reimbursement).to receive(:save).and_return(false)
            allow_any_instance_of(Aypex::Reimbursement).to receive(:errors) { double(full_messages: "foo") }
          end
  ...
  describe "whitelisted_ransackable_attributes" do
    it { expect(Aypex::ReturnAuthorization.whitelisted_ransackable_attributes).to eq(%w[memo number state]) }
  end

  describe "whitelisted_ransackable_associations" do
    it { expect(Aypex::ReturnAuthorization.whitelisted_ransackable_associations).to eq(%w[order]) }
  end
  ...
  describe "#display_pre_tax_total" do
    it "returns a Aypex::Money" do
      ...
      expect(return_authorization.display_pre_tax_total).to eq(Aypex::Money.new(21.22))
    end
  end
