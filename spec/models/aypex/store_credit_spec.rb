...
describe Aypex::StoreCredit, type: :model do
  ...
  describe "callbacks" do
    ...
    context "amount used is greater than zero" do
      ...
      let(:validation_message) { I18n.t("activerecord.errors.models.aypex/store_credit.attributes.amount_used.greater_than_zero_restrict_delete") }
  ...
  describe "validations" do
    describe "used amount should not be greater than the credited amount" do
      context "the used amount is defined" do
        ...
        it "sets the correct error message" do
          ...
          attribute_name = I18n.t("activerecord.attributes.aypex/store_credit.amount_used")
          validation_message = I18n.t("activerecord.errors.models.aypex/store_credit.attributes.amount_used.cannot_be_greater_than_amount")
    ...
    describe "amount used less than or equal to amount" do
      ...
      it "adds an error message about the invalid amount used" do
        subject.valid?
        text = I18n.t("activerecord.errors.models.aypex/store_credit.attributes.amount_used.cannot_be_greater_than_amount")
    ...
    describe "amount authorized less than or equal to amount" do
      ...
      it "adds an error message about the invalid authorized amount" do
        subject.valid?
        text = I18n.t("activerecord.errors.models.aypex/store_credit.attributes.amount_authorized.exceeds_total_credits")
  ...
  describe "#display_amount" do
    it "returns a Aypex::Money instance" do
      expect(store_credit.display_amount).to be_instance_of(Aypex::Money)
    end
  end

  describe "#display_amount_used" do
    it "returns a Aypex::Money instance" do
      expect(store_credit.display_amount_used).to be_instance_of(Aypex::Money)
    end
  end
      ...
      context "amount has not been authorized yet" do
        ...
        context "originator is present" do
          ...
          it "records the originator" do
            expect { subject }.to change { Aypex::StoreCreditEvent.count }.by(1)
            expect(Aypex::StoreCreditEvent.last.originator).to eq originator
          end
        end
      end
    ...
    context "amount is successfully credited" do
      ...
      context "credit_to_new_allocation is set" do
        ...
        it "creates a new store credit record" do
          expect { subject }.to change { Aypex::StoreCredit.count }.by(1)
        end
        ...
        context "credits the passed amount to a new store credit record" do
          before do
            subject
            @new_store_credit = Aypex::StoreCredit.last
          end
  ...
  describe "#can_capture?" do
    ...
    context "void payment" do
      let(:payment_state) { Aypex::StoreCredit::VOID_ACTION }
      ...
      context "create" do
        context "user has one store credit" do
          ...
          it "makes the store credit event an allocation event" do
            expect(subject.store_credit_events.first.action).to eq Aypex::StoreCredit::ALLOCATION_ACTION
          end
        ...
        context "an action is specified" do
          it "creates an event with the set action" do
            ...
            store_credit.action = Aypex::StoreCredit::VOID_ACTION
            store_credit.action_authorization_code = "1-SC-TEST"

            expect { store_credit.save! }.to change {
              Aypex::StoreCreditEvent.where(action: Aypex::StoreCredit::VOID_ACTION).count
            }.by(1)
          end
        end
      end
