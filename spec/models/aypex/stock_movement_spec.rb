...
describe Aypex::StockMovement do
  describe "Constants" do
    describe "QUANTITY_LIMITS[:max]" do
      it "return 2**31 - 1" do
        expect(Aypex::StockMovement::QUANTITY_LIMITS[:max]).to eq(2**31 - 1)
      end
    end

    describe "QUANTITY_LIMITS[:min]" do
      it "return -2**31" do
        expect(Aypex::StockMovement::QUANTITY_LIMITS[:min]).to eq(-2**31)
      end
    end
  end
  ...
  describe "Scope" do
    describe ".recent" do
      it "orders chronologically by created at" do
        expect(Aypex::StockMovement.recent.to_sql)
          .to eq Aypex::StockMovement.unscoped.order(created_at: :desc).to_sql
      end
    end
  end
  ...
  describe "whitelisted ransackable attributes" do
    it "returns amount attribute" do
      expect(Aypex::StockMovement.whitelisted_ransackable_attributes).to eq(["quantity"])
    end
  end
