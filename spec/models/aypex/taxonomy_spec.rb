...
describe Aypex::Taxonomy, type: :model do
  ...
  describe "#destroy" do
    ...
    it "destroys all associated taxons" do
      @taxonomy.destroy
      expect { Aypex::Taxon.find(@root_taxon.id) }.to raise_error(ActiveRecord::RecordNotFound)
      expect { Aypex::Taxon.find(@child_taxon.id) }.to raise_error(ActiveRecord::RecordNotFound)
    end
  end
