...
describe Aypex::CustomerReturn, type: :model do
  before do
  ...
  describe "whitelisted_ransackable_attributes" do
    it { expect(Aypex::CustomerReturn.whitelisted_ransackable_attributes).to eq(%w[number]) }
  end
  ...
  describe "#display_pre_tax_total" do
    ...
    before do
      Aypex::ReturnItem.where(customer_return_id: customer_return.id).update_all(pre_tax_amount: pre_tax_amount)
    end
  ...
  describe "#display_pre_tax_total" do
    let(:customer_return) { Aypex::CustomerReturn.new }

    it "returns a Aypex::Money" do
      allow(customer_return).to receive_messages(pre_tax_total: 21.22)
      expect(customer_return.display_pre_tax_total).to eq(Aypex::Money.new(21.22))
    end
  end
    ...
    context "to a different stock location" do
      ...
      it "updates the stock item counts in new stock location" do
        expect do
          ...
          Aypex::StockItem.where(variant_id: inventory_unit.variant_id, stock_location_id: new_stock_location.id).first.count_on_hand
        }.by(1)
      end
