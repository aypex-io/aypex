...
module Aypex
  module Stock
    describe Estimator, type: :model do
      ...
      let(:order) { build(:order_with_line_items, ship_address: ship_address, store: store) }
      ...
      describe "#shipping rates" do
        before do
          ...
          allow_any_instance_of(ShippingMethod).to receive_message_chain(:calculator, :currency).and_return(currency)
        ...
        it "sorts shipping rates by cost" do
          shipping_methods = create_list(:shipping_method, 3)
        ...
        context "general shipping methods" do
          let(:shipping_methods) { create_list(:shipping_method, 2) }
        ...
        context "involves admin only shipping methods" do
          let(:admin_method) { create(:shipping_method, display_on: "back_end") }
          let(:generic_method) { create(:shipping_method) }

          before do
            allow(admin_method).to receive_message_chain(:calculator, :compute).and_return(0.00)
            allow(generic_method).to receive_message_chain(:calculator, :compute).and_return(5.00)
            allow(package).to receive(:shipping_methods).and_return([admin_method, generic_method])
          end

          it "does not return admin rates at all" do
            expect(subject.shipping_rates(package).map(&:shipping_method_id)).to eq([generic_method.id])
          end

          # regression for #3287
          it "doesn't select admin rates even if they're more affordable" do
        ...
        context "includes tax adjustments if applicable" do
          ...
          before do
            Aypex::ShippingMethod.all.each do |sm|
