...
describe Aypex::Promotion do
    ...
    describe "expires_at_must_be_later_than_starts_at" do
      ...
      context "starts_at is a date earlier than expires_at" do
        ...
        context "is not valid" do
          ...
          it { expect(valid_promotion.errors[:expires_at]).to include(I18n.t(:invalid_date_range, scope: "activerecord.errors.models.aypex/promotion.attributes.expires_at")) }
        end
      end
  ...
  describe "scopes" do
    describe ".coupons" do
      ...
      let!(:promotion_with_code) { create(:promotion, name: "test-1", code: "code") }
    ...
    describe ".applied" do
      ...
      let!(:promotion_applied) do
        promotion = create(:promotion, name: "test-1", code: "")
    ...
    describe ".advertised" do
      ...
      let!(:promotion_advertised) { create(:promotion, name: "test-1", advertise: true) }
  ...
  describe "#destroy" do
    ...
    it "deletes actions" do
      expect(Aypex::PromotionAction.count).to eq(0)
    end

    it "deletes rules" do
      expect(Aypex::PromotionRule.count).to eq(0)
    end
  end
  ...
  describe "#save" do
    ...
    it "deeply auto saves records and preferences" do
      promotion.actions[0].calculator.percent = 10
      promotion.save!
      expect(Aypex::Calculator.first.percent).to eq(10)
    end
  end

  describe "#activate" do
    let(:action1) { Aypex::Promotion::Actions::CreateItemAdjustments.create! }
    let(:action2) { Aypex::Promotion::Actions::CreateItemAdjustments.create! }
  ...
  describe "#credits_count" do
    ...
    let(:promotion) { create(:promotion, name: "promo-x", code: "PROMOX") }

    let(:item_action) do
      action = Aypex::Promotion::Actions::CreateItemAdjustments.create(calculator: Aypex::Calculator::Promotion::PercentOnLineItem.new)
      promotion.actions << action
      action
    end

    let(:item_adjustment) do
      Aypex::Adjustment.create!(
        source: item_action,
        amount: 10,
    ...
    it "counts eligible adjustments" do
      item_adjustment.update_column(:eligible, true)
  ...
  describe "#adjusted_credits_count" do
    let(:order) { create(:order) }
    let(:line_item) { create(:line_item, order:) }
    let(:promotion) { create(:promotion, name: "promo", code: "10-off") }
  ...
  describe "#products" do
    ...
    context "when it has product rules with products associated" do
      let(:promotion_rule) { create(:promotion_rule, promotion:, type: "Aypex::Promotion::Rules::Product") }
  ...
  describe "#eligible?" do
    ...
    context "when promotable is a Aypex::LineItem" do
    ...
    context "when promotable is a Aypex::Order" do
      let(:promotable) { create(:order) }
  ...
  describe "#eligible_rules" do
    ...
    context "when there are no applicable rules" do
      let!(:promotion_rule) { create(:promotion_rule, promotion:) }
    ...
    context "with 'all' match policy" do
      let(:promo1) { Aypex::PromotionRule.create! }
      let(:promo2) { Aypex::PromotionRule.create! }
      ...
      context "when all rules are eligible" do
        ...
        it "does set anything to eligibility errors" do
    ...
    context "with 'any' match policy" do
      ...
      it "has eligible rules if any of the rules are eligible" do
        allow_any_instance_of(Aypex::PromotionRule).to receive_messages(applicable?: true)
        true_rule = Aypex::PromotionRule.create(promotion:)
      ...
      context "when none of the rules are eligible" do
        let(:promo) { Aypex::PromotionRule.create! }
        let(:errors) { double ActiveModel::Errors, empty?: false }
  ...
  describe "#line_item_actionable?" do
    ...
    let(:order) { double Aypex::Order }
    let(:line_item) { double Aypex::LineItem }
    let(:true_rule) { double Aypex::PromotionRule, eligible?: true, applicable?: true, actionable?: true }
    let(:false_rule) { double Aypex::PromotionRule, eligible?: true, applicable?: true, actionable?: false }
    let(:rules) { [] }
  ...
  describe "#with_coupon_code" do
    context "and code stored in uppercase" do
      let!(:promotion) { create(:promotion, :with_line_item_adjustment, code: "MY-COUPON-123") }
    ...
    context "when promotion has no actions" do
      ...
      let!(:promotion_with_actions) { create(:promotion_with_item_adjustment, code: "MY-COUPON-123") }
      ...
      it "return the last one created" do
        promotion_with_actions_2 = create(:promotion_with_item_adjustment, code: "MY-COUPON-123")
  ...
  describe "adding items to the cart" do
    ...
    it "updates the promotions for new line items" do
      ...
      promo.activate(order:)
      order.update_with_updater!

      expect(line_item.adjustments.size).to eq(1)
      expect(order.adjustment_total).to eq(-5)

      other_line_item = Aypex::Cart::AddItem.call(order:, variant:, options: {currency: order.currency}).value
  ...
  describe "#generate_code" do
    let(:promotion) { create(:promotion, code: "aypex-123") }

    context "with generate_code" do
      it "has a generated code" do
        promotion.generate_code = true
        expect(promotion.code).not_to eq "aypex-123"
      end
    end

    context "without generate_code" do
      it "has a generated code" do
        expect(promotion.code).to eq "aypex-123"
      end
    end
  end
end
