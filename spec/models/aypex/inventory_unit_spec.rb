...
describe Aypex::InventoryUnit, type: :model do
  ...
  describe "scopes" do
    ...
    describe ".backordered" do
      it { expect(Aypex::InventoryUnit.backordered).to eq([inventory_unit_2]) }
    end

    describe ".on_hand" do
      it { expect(Aypex::InventoryUnit.on_hand).to eq([inventory_unit_1]) }
    end

    describe ".on_hand_or_backordered" do
      it { expect(Aypex::InventoryUnit.on_hand_or_backordered).to match_array([inventory_unit_1, inventory_unit_2]) }
    end

    describe ".shipped" do
      it { expect(Aypex::InventoryUnit.shipped).to eq([inventory_unit_3]) }
    end

    describe ".returned" do
      it { expect(Aypex::InventoryUnit.returned).to eq([inventory_unit_4]) }
    end
  end
  ...
  describe "#backordered_for_stock_item" do
    ...
    it "returns modifiable objects" do
      units = Aypex::InventoryUnit.backordered_for_stock_item(stock_item)
    ...
    it "finds inventory units from its stock location when the unit's variant matches the stock item's variant" do
      expect(Aypex::InventoryUnit.backordered_for_stock_item(stock_item)).to match_array([unit])
    end
    ...
    it "does not find inventory units that aren't backordered" do
      ...
      expect(Aypex::InventoryUnit.backordered_for_stock_item(stock_item)).not_to include(on_hand_unit)
    end
    ...
    it "does not find inventory units that don't match the stock item's variant" do
      ...
      expect(Aypex::InventoryUnit.backordered_for_stock_item(stock_item)).not_to include(other_variant_unit)
    end

    it "does not change shipping cost when fulfilling the order" do
      current_shipment_cost = shipment.cost
      shipping_method.calculator.amount = (current_shipment_cost + 5.0)
    ...
    context "other shipments" do
      ...
      it "does not find inventory units belonging to incomplete orders" do
        expect(Aypex::InventoryUnit.backordered_for_stock_item(stock_item)).not_to include(other_unit)
      end
    end
  end
