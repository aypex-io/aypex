...
describe Aypex::TaxRate do
  context "match" do
    ...
    let(:calculator) { Aypex::Calculator::Promotion::FlatRate.new }

    it "returns an empty array when tax_zone is nil" do
      allow(order).to receive_messages tax_zone: nil
      expect(Aypex::TaxRate.match(order.tax_zone)).to eq([])
    end

    context "when no rate zones match the tax zone" do
      before do
        Aypex::TaxRate.create(name: "Tax Rate #1", amount: 1, zone: create(:zone))
      end
      ...
      context "when there is no default tax zone" do
        ...
        it "returns the rate that matches the rate zone" do
          ...
          allow(order).to receive_messages tax_zone: @zone
          expect(Aypex::TaxRate.match(order.tax_zone)).to eq([rate])
        end

        it "returns all rates that match the rate zone" do
          rate1 = Aypex::TaxRate.create(
            name: "Tax Rate #1",
            amount: 1,
            zone: @zone,
            tax_category: tax_category,
          ...
          allow(order).to receive_messages tax_zone: @zone
          expect(Aypex::TaxRate.match(order.tax_zone)).to match_array([rate1, rate2])
        end
      ...
      context "when there is a default tax zone" do
        subject { Aypex::TaxRate.match(order.tax_zone) }
        ...
        let!(:rate) do
          Aypex::TaxRate.create(name: "Tax Rate #1",
            amount: 1,
            zone: @zone,
            tax_category: tax_category,
            calculator: calculator,
  ...
  describe ".adjust" do
    ...
    context "with line items" do
      ...
      before { allow(Aypex::TaxRate).to receive_messages match: [rate_1, rate_2] }
      ...
      it "applies adjustments for two tax rates to the order" do
        ...
        Aypex::TaxRate.adjust(order, line_items)
      end
    end
    ...
    context "with shipments" do
      ...
      before do
        allow(Aypex::TaxRate).to receive_messages match: [rate_1, rate_2]
      ...
      it "applies adjustments for two tax rates to the order" do
        expect(rate_1).to receive(:adjust)
        expect(rate_2).not_to receive(:adjust)
        Aypex::TaxRate.adjust(order, [shipment])
      end
    end
    ...
    context "for MOSS taxation in Europe" do
      ...
      let(:moss_category) { Aypex::TaxCategory.create(name: "Digital Goods") }
      let(:normal_category) { Aypex::TaxCategory.create(name: "Analogue Goods") }
      ...
      context "a download" do
        before do
          Aypex::Cart::AddItem.call(order: order, variant: download.master)
        end

        it "without an address costs 100 euros including tax" do
          Aypex::TaxRate.adjust(order, order.line_items)
          order.update_with_updater!
          expect(order.display_total).to eq(Aypex::Money.new(100))
        ...
        it "to france costs more including tax" do
          ...
          Aypex::TaxRate.adjust(order, order.line_items)
          order.update_with_updater!
          expect(order.display_total).to eq(Aypex::Money.new(105.04))
        ...
        it "to somewhere else costs the net amount" do
          ...
          expect(order.included_tax_total).to eq(0)
          expect(order.display_total).to eq(Aypex::Money.new(84.03))
        end
      end
      ...
      context "a t-shirt" do
        it "to germany costs 100 euros including tax" do
          allow(order).to receive(:tax_zone).and_return(germany_zone)
          Aypex::Cart::AddItem.call(order: order, variant: tshirt.master)
          Aypex::TaxRate.adjust(order, order.line_items)
          order.update_with_updater!
          expect(order.display_total).to eq(Aypex::Money.new(100))
          expect(order.included_tax_total).to eq(15.97)
        ...
        it "to france costs 100 euros including tax" do
          ...
          Aypex::TaxRate.adjust(order, order.line_items)
          order.update_with_updater!
          expect(order.display_total).to eq(Aypex::Money.new(100.00))
          expect(order.included_tax_total).to eq(15.97)
          expect(order.additional_tax_total).to eq(0)
        end
  ...
  describe ".included_tax_amount_for" do
    subject(:included_tax_amount) { Aypex::TaxRate.included_tax_amount_for(price_options) }
  ...
  describe "#adjust" do
    before do
      ...
      @zone = create(:zone, name: "Country Zone", kind: "country", default_tax: true, zone_members: [])
      @zone.zone_members.create(zoneable: @country)
      @category = Aypex::TaxCategory.create name: "Taxable Foo"
      @category2 = Aypex::TaxCategory.create(name: "Non Taxable")
      @rate1 = Aypex::TaxRate.create(
        name: "Tax Rate #1",
        amount: 0.10,
        calculator: Aypex::Calculator::Tax::Default.create,
    ...
    context "not taxable line item" do
      let!(:line_item) { Aypex::Cart::AddItem.call(order: @order, variant: @nontaxable.master).value }
    ...
    context "taxable line item" do
      let!(:line_item) { Aypex::Cart::AddItem.call(order: @order, variant: @taxable.master).value }

      context "when price includes tax" do
        before do
          @rate1.update_column(:included_in_price, true)
          @rate2.update_column(:included_in_price, true)
          Aypex::TaxRate.store_pre_tax_amount(line_item, [@rate1, @rate2])
        end
        ...
        context "when price does not include tax" do
          ...
          describe "tax adjustments" do
            before { Aypex::TaxRate.adjust(@order, @order.line_items) }
