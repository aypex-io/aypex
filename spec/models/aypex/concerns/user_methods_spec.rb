...
describe Aypex::UserMethods do
  let(:test_user) { create(:user) }
  let(:current_store) { create(:store) }

  describe "#has_aypex_role?" do
    subject { test_user.has_aypex_role? name }

    let(:role) { Aypex::Role.create(name: name) }
    let(:name) { "test" }

    context "with a role" do
      before { test_user.aypex_roles << role }
  ...
  describe "#aypex_admin?" do
    it do
      ...
      expect(create(:user).aypex_admin?).to be false
    end
  end
  ...
  describe "#last_incomplete_aypex_order" do
    subject { test_user.last_incomplete_aypex_order(current_store) }
  ...
  describe "#check_completed_orders" do
    ...
    context "rstrict t delete dependent destroyed" do
      ...
      it "does not destroy dependent destroy items" do
        expect { test_user.destroy }.to raise_error(Aypex::DestroyWithOrdersError)
