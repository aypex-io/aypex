...
describe Aypex::ReturnItem, type: :model do
  ...
  before do
    allow_any_instance_of(Aypex::Order).to receive_messages(return!: true)
  end
  ...
  describe "#display_pre_tax_amount" do
    ...
    it "returns a Aypex::Money" do
      expect(return_item.display_pre_tax_amount).to eq(Aypex::Money.new(pre_tax_amount))
    end
  end

  describe ".default_refund_amount_calculator" do
    it "defaults to the default refund amount calculator" do
      expect(Aypex::ReturnItem.refund_amount_calculator).to eq Aypex::Calculator::Returns::DefaultRefundAmount
    end
  end
  ...
  describe "pre_tax_amount calculations on create" do
    ...
    context "pre tax amount is not specified" do
      ...
      context "not an exchange" do
        it { expect(subject.pre_tax_amount).to eq Aypex::Calculator::Returns::DefaultRefundAmount.new.compute(subject) }
      end
  ...
  describe ".from_inventory_unit" do
    subject { Aypex::ReturnItem.from_inventory_unit(inventory_unit) }
  ...
  describe "validity for reimbursements" do
    ...
    context "when acceptance_status is accepted" do
      ...
      it "is valid" do
        ...
        expect(subject.errors.messages).to eq(reimbursement: [I18n.t(:cannot_be_associated_unless_accepted, scope: "activerecord.errors.models.aypex/return_item.attributes.reimbursement")])
      end
    end
  end
  ...
  describe "#eligible_exchange_variants" do
    it "uses the exchange variant calculator to compute possible variants to exchange for" do
      ...
      expect(Aypex::ReturnItem.exchange_variant_engine).to receive(:eligible_variants).with(return_item.variant)
  ...
  describe ".exchange_variant_engine" do
    it "defaults to the same product calculator" do
      expect(Aypex::ReturnItem.exchange_variant_engine).to eq Aypex::ReturnItem::ExchangeVariantEligibility::SameProduct
    end
  end
  ...
  describe "#process_inventory_unit!" do
    ...
    context "stock should restock" do
      ...
      it { expect(subject).to eq(Aypex::StockMovement.find_by(stock_movement_attributes)) }
    end
