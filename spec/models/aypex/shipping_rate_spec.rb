...
describe Aypex::ShippingRate do
  ...
  let(:shipping_rate) do
    Aypex::ShippingRate.new shipment: shipment,
  ...
  describe "#display_price" do
    ...
    context "when the currency is JPY" do
      let(:shipping_rate) { Aypex::ShippingRate.new(cost: 205) }
  ...
  describe "#final_price" do
    ...
    it "returns 0 if cost is lesser than the discount amount" do
      allow_any_instance_of(Aypex::ShippingRate).to receive_messages(discount_amount: -20.0)
      expect(shipping_rate.final_price).to eq(0.0)
    end

    it "returns cost minus discount amount" do
      allow_any_instance_of(Aypex::ShippingRate).to receive_messages(discount_amount: -5.0)
