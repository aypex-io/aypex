...
describe Aypex::Payment do
  ...
  let(:gateway) do
    gateway = Aypex::Gateway::Bogus.new(active: true, stores: [store])
  ...
  describe "Constants" do
    it { expect(Aypex::Payment::INVALID_STATES).to eq(%w[failed invalid void]) }
  end

  describe "scopes" do
    describe ".valid" do
      subject { Aypex::Payment.valid }
  ...
  context "validations" do
    ...
    it "returns useful error messages when source is invalid" do
      payment.source = Aypex::CreditCard.new
      expect(payment).not_to be_valid
  ...
  # Regression test for https://github.com/aypex/aypex/pull/2224
  context "failure" do
      ...
      context "if unsuccessful" do
        context "when response is returned from gateway" do
          it "marks payment as failed" do
            ...
            expect do
              payment.authorize!
            end.to raise_error(Aypex::GatewayError)
          end
        end
        ...
        context "when there is an error connecting to the gateway" do
          ...
          it "raises ActiveMerchant::ConnectionError and marks payment as failed" do
            expect(payment).to receive(:failure!)
            expect(payment).not_to receive(:pend)
            expect { payment.authorize! }.to raise_error(Aypex::GatewayError)
          end
        end
      end
    end
      ...
      context "if unsuccessful" do
        context "when response is returned from gateway" do
          ...
          it "makes payment failed" do
            expect { payment.purchase! }.to raise_error(Aypex::GatewayError)
          end
      ...
      context "when payment is pending" do
        ...
        context "if successful" do
          ...
          context "for partial amount" do
            ...
            before do
              allow_any_instance_of(Aypex::Payment).to receive(:payment_method_available_for_order).and_return(nil)
        ...
        context "if unsuccessful" do
          context "when response is returned from gateway" do
            it "does not make payment complete" do
              ...
              expect { payment.capture! }.to raise_error(Aypex::GatewayError)
            end
          end
      ...
      context "if unsuccessful" do
        context "when response is returned from gateway" do
          it "does not void the payment" do
            ...
            expect { payment.void_transaction! }.to raise_error(Aypex::GatewayError)
          end
        end
  ...
  describe "#save" do
    ...
    context "not completed payments" do
      it "doesn't update order payment total" do
        expect do
          Aypex::Payment.create(amount: 100, order: order)
        end.not_to change(order, :payment_total)
      end

      it "requires a payment method" do
        expect(Aypex::Payment.create(amount: 100, order: order).errors).not_to be_empty
        expect(Aypex::Payment.create(amount: 100, order: order).errors.messages[:payment_method]).to be_present
      end
    end

    context "when the payment was completed but now void" do
      let(:payment) do
        Aypex::Payment.create(
          amount: 100,
          order: order,
          state: "completed"
        )
    ...
    context "completed orders" do
      ...
      it "updates payment_state and shipments" do
        ...
        Aypex::Payment.create(amount: 100, order: order, payment_method: gateway, source: card)
      end
    end
    ...
    context "when profiles are supported" do
      ...
      context "with multiple payment attempts" do
        ...
        it "does not try to create profiles on old failed payment attempts" do
          allow_any_instance_of(Aypex::Payment).to receive(:payment_method) { gateway }
  ...
  describe "#build_source" do
    ...
    it "builds the payment's source" do
      payment = Aypex::Payment.new(params)
      expect(payment).to be_valid
  ...
  describe "#display_amount" do
    it "returns a Aypex::Money for this amount" do
      expect(payment.display_amount).to eq(Aypex::Money.new(payment.amount))
    end
  end
  ...
  describe "#gateway_options" do
    ...
    it "contains the email address from a persisted order" do
      ...
      payment.order = Aypex::Order.find(payment.order_id)
