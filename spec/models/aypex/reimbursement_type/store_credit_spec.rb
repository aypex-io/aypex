...
module Aypex
  describe ReimbursementType::StoreCredit do
    subject do
      Aypex::ReimbursementType::StoreCredit.reimburse(reimbursement, [return_item, return_item2],
        simulate)
    end
    ...
    let!(:default_refund_reason) do
      Aypex::RefundReason.find_or_create_by!(name: Aypex::RefundReason::RETURN_PROCESSING_REASON,
        mutable: false)
    end

    let!(:primary_credit_type) { create(:primary_credit_type) }
    let!(:created_by_user) { create(:user, email: Aypex::StoreCredit::DEFAULT_CREATED_BY_EMAIL) }
    ...
    describe ".reimburse" do
      context "simulate is true" do
        ...
        context "for store credits that the customer used" do
          ...
          it "creates readonly refunds for all store credit payments" do
            expect(subject.map(&:class)).to eq [Aypex::Refund]
      ...
      context "simulate is false" do
        ...
        context "for return items that were not paid for with store credit" do
          before do
            allow(Aypex::ReimbursementType::StoreCredit).to receive(:store_credit_payments).and_return([])
          end
          ...
          it "creates a store credit with the same currency as the reimbursement's order" do
            ...
            expect(Aypex::StoreCredit.last.currency).to eq reimbursement.order.currency
          end
        end
      end
    end
