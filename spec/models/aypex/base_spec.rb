...
describe Aypex::Base do
  context "AR overrides", skip: ENV["DB"] == "mysql" do
    ...
    it "does not override Rails 5 default belongs_to_required_by_default" do
      ...
      expect(Aypex::Product.belongs_to_required_by_default).to be(false)
    ...
    it "does not disable non-aypex, Rails 5 models to validate their associated belongs_to model" do
  ...
  describe ".json_api_type" do
    it { expect(Aypex::InventoryUnit.json_api_type).to eq("inventory_unit") }
    it { expect(Aypex::Address.json_api_type).to eq("address") }
  end

  describe ".json_api_columns" do
    it "skips sensitive data" do
      expect(Aypex::LegacyUser.json_api_columns).not_to include("password")
      expect(Aypex::LegacyUser.json_api_columns).to include("email")
    end

    it { expect(Aypex::Address.json_api_columns).to contain_exactly("address1", "address2", "alternative_phone", "city", "company", "created_at", "deleted_at", "firstname", "label", "lastname", "phone", "state_name", "updated_at", "zipcode", "public_metadata", "private_metadata") }
    it { expect(Aypex::Address.json_api_columns).not_to include("country_id") }
  end

  describe ".json_api_permitted_attributes" do
    it { expect(Aypex::Address.json_api_permitted_attributes).to contain_exactly("firstname", "lastname", "address1", "address2", "city", "zipcode", "phone", "state_name", "alternative_phone", "company", "state_id", "country_id", "created_at", "updated_at", "user_id", "deleted_at", "label", "public_metadata", "private_metadata") }
  end
end
