  ...
  describe "#last_incomplete_order" do
    ...
    it "returns correct order" do
      Timecop.scale(3600) do
        ...
        expect(user.last_incomplete_aypex_order(store)).to eq order_3
      end
    end
    ...
    context "persists order address" do
      it "copies over order addresses" do
        ...
        end.to change { Aypex::Address.count }.by(2)
      ...
      it "doesnt create new addresses if user has already" do
        ...
        expect do
          user.persist_order_address(order)
        end.not_to change { Aypex::Address.count }
      end
    ...
    context "user has several associated store credits" do
      ...
      context "store credits have never been used" do
        it "returns store credit amount" do
          expect(subject.available_store_credits(store)).to eq([Aypex::Money.new(usd_amount, currency: "USD")])
        end
      end
      ...
      context "store credits in different currencies exits" do
        ...
        it "returns sum of amounts" do
          expect(subject.available_store_credits(store)).to match_array([Aypex::Money.new(usd_amount, currency: "USD"),
            Aypex::Money.new(gbp_amount, currency: "GBP"),
            Aypex::Money.new(eur_amount, currency: "EUR")])
        end
      end
    end
  end
