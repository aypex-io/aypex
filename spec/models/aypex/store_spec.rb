...
describe Aypex::Store do
  ...
  describe ".by_url" do
    ...
    it "finds stores by url" do
      by_domain = Aypex::Store.by_url("www.subdomain.com")
  ...
  describe ".default" do
    context "when a default store is already present" do
      ...
      it "returns the already existing default store" do
        expect(Aypex::Store.default).to eq(store_2)
      end
      ...
      it "ensures there is only one default" do
        ...
        expect(Aypex::Store.where(default: true).count).to eq(1)
        expect(store_2.default).to be true
  ...
  shared_context "with default checkout zone set" do
    ...
    let(:default_zone) do
      Aypex::Zone.first || create(:zone, kind: "country").tap do |zone|
        zone.members.create(zoneable: country3)
      end
    end

    before { allow(Aypex::Zone).to receive(:default_checkout_zone) { default_zone } }
  end

  shared_context "with default checkout zone not set" do
    before { allow(Aypex::Zone).to receive(:default_checkout_zone).and_return(nil) }
  end
  ...
  describe "#countries_available_for_checkout" do
    ...
    context do
      ...
      it "returns list of all countries" do
        ...
        all_countries_ids = Aypex::Country.all.ids
  ...
  describe "#ensure_default_country" do
    ...
    let!(:default_country) { Aypex::Country.first || create(:country) }
