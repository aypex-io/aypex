...
describe Aypex::Promotion::Rules::OptionValue do
  let(:rule) { described_class.new }

  describe "#eligible_values" do
    subject { rule.eligible_values }

    it "assigns a nicely formatted hash" do
      rule.eligible_values = ({"5" => "1,2", "6" => "1"})
  ...
  describe "#applicable?" do
    ...
    context "when promotable is an order" do
      let(:promotable) { Aypex::Order.new }

      it { is_expected.to be true }
    end

    context "when promotable is not an order" do
      let(:promotable) { Aypex::LineItem.new }
  ...
  describe "#eligible?" do
    ...
    context "when there are any applicable line items" do
      before do
        rule.eligible_values = ({variant.product.id => [
  ...
  describe "#actionable?" do
    ...
    before do
      ...
      rule.eligible_values = ({product_id => option_value_ids})
    end
