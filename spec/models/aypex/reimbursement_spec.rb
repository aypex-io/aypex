...
describe Aypex::Reimbursement, type: :model do
  describe "#display_total" do
    ...
    it "returns the value as a Aypex::Money instance" do
      expect(subject).to eq Aypex::Money.new(total)
    end
  ...
  describe "#perform!" do
    ...
    it "creates a refund" do
      expect do
        subject
      end.to change { Aypex::Refund.count }.by(1)
      expect(Aypex::Refund.last.amount).to eq order.total
    end
    ...
    context "with additional tax" do
      ...
      it "saves the additional tax and refunds the total" do
        ...
        expect(Aypex::Refund.last.amount).to eq line_item.pre_tax_amount + line_item.additional_tax_total
      end
    end
    ...
    context "with included tax" do
      ...
      it "saves the included tax and refunds the total" do
        ...
        expect(Aypex::Refund.last.amount).to eq (line_item.pre_tax_amount + line_item.included_tax_total).round(2)
      end
    end
    ...
    context "when reimbursement cannot be fully performed" do
      ...
      it "raises IncompleteReimbursement error" do
        expect { subject }.to raise_error(Aypex::Reimbursement::IncompleteReimbursementError)
      end
    end
  ...
  describe "#calculated_total" do
    context "with return item amounts that would round up if added" do
      ...
      let(:reimbursement) { Aypex::Reimbursement.new }

      before do
        reimbursement.return_items << Aypex::ReturnItem.new(pre_tax_amount: 10.003)
        reimbursement.return_items << Aypex::ReturnItem.new(pre_tax_amount: 10.003)
      end
    ...
    context "with a return item amount that should round up" do
      ...
      before do
        reimbursement.return_items << Aypex::ReturnItem.new(pre_tax_amount: 19.998)
      end
  ...
  describe ".build_from_customer_return" do
    subject { Aypex::Reimbursement.build_from_customer_return(customer_return) }
