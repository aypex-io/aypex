...
class FakeCalculator < Aypex::Calculator
...
describe Aypex::Order, type: :model do
  ...
  before { allow(Aypex::LegacyUser).to receive_messages(current: create(:user)) }
  ...
  describe ".scopes" do
    ...
    describe ".complete" do
      it { expect(Aypex::Order.complete).to include completed_order }
      it { expect(Aypex::Order.complete).not_to include incompleted_order }
    end

    describe ".incomplete" do
      it { expect(Aypex::Order.incomplete).to include incompleted_order }
      it { expect(Aypex::Order.incomplete).not_to include completed_order }
    end
  end
  ...
  describe "#cancel" do
    ...
    it "marks the payments as void" do
      allow_any_instance_of(Aypex::Shipment).to receive(:refresh_rates).and_return(true)
      order.cancel
      order.reload
  ...
  describe "#finalize!" do
    let(:order) { Aypex::Order.create(email: "test@example.com", store: store) }
    ...
    it "changes the shipment state to ready if order is paid" do
      Aypex::Shipment.create(order: order, stock_location: create(:stock_location))
  ...
  describe "#display_outstanding_balance" do
    it "returns the value as a aypex money" do
      allow(order).to receive(:outstanding_balance).and_return(10.55)
      expect(order.display_outstanding_balance).to eq(Aypex::Money.new(10.55))
    end
  end
  ...
  describe "#display_item_total" do
    it "returns the value as a aypex money" do
      ...
      expect(order.display_item_total).to eq(Aypex::Money.new(10.55))
    end
  end
  ...
  describe "#display_adjustment_total" do
    it "returns the value as a aypex money" do
      ...
      expect(order.display_adjustment_total).to eq(Aypex::Money.new(10.55))
    end
  end
  ...
  describe "#display_promo_total" do
    it "returns the value as a aypex money" do
      ...
      expect(order.display_promo_total).to eq(Aypex::Money.new(10.55))
    end
  end
  ...
  describe "#display_total" do
    it "returns the value as a aypex money" do
      order.total = 10.55
      expect(order.display_total).to eq(Aypex::Money.new(10.55))
    end
  end
  ...
  describe "#confirmation_required?" do
    ...
    it "is required if the state is currently 'confirm'" do
      order = Aypex::Order.new(store: store)
    ...
    context "order.store.checkout_always_include_confirm_step = true" do
      it "returns true if payments empty" do
        order = Aypex::Order.new(store: store)

        order.store.checkout_always_include_confirm_step = true
        assert order.confirmation_required?
      end
    end

    context "order.store.checkout_always_include_confirm_step == false" do
      it "returns false if payments empty and order.store.checkout_always_include_confirm_step == false" do
        order = Aypex::Order.new(store: store)
        assert !order.confirmation_required?
      end
  ...
  context "add_update_hook" do
    ...
    after do
      Aypex::Order.update_hooks = Set.new
    end
  ...
  describe "#tax_address" do
    ...
    before { order.store.tax_using_ship_address = tax_using_ship_address }
  ...
  describe "#available_payment_methods" do
    ...
    it "includes frontend payment methods" do
      payment_method = Aypex::PaymentMethod.create!(name: "Fake",
        active: true,
    ...
    it "does not include a payment method that is not suitable for this order" do
      allow(Aypex::PaymentMethod).to receive(:available_on_front_end).and_return(methods)
  ...
  describe "#apply_free_shipping_promotions" do
    it "calls out to the FreeShipping promotion handler" do
      ...
      expect(Aypex::PromotionHandler::FreeShipping).to receive(:new).and_return(handler = double)
      expect(handler).to receive(:activate)

      expect(Aypex::Adjustable::AdjustmentsUpdater).to receive(:update).with(shipment)

      expect(Aypex::TaxRate).to receive(:adjust).with(order, [shipment])
  ...
  describe "#display_pre_tax_item_amount" do
    it "returns the value as a aypex money" do
      ...
      expect(order.display_pre_tax_item_amount).to eq(Aypex::Money.new(10.55))
    end
  end
  ...
  describe "#display_pre_tax_total" do
    it "returns the value as a aypex money" do
      ...
      expect(order.display_pre_tax_total).to eq(Aypex::Money.new(10.55))
    end
  end
  ...
  describe "#create_proposed_shipments" do
    ...
    it "assigns the coordinator returned shipments to its shipments" do
      ...
      allow_any_instance_of(Aypex::Stock::Coordinator).to receive(:shipments).and_return([shipment])
      subject.create_proposed_shipments
  ...
  describe "#fully_discounted?" do
    let(:line_item) { Aypex::LineItem.new(price: 10, quantity: 1) }
    let(:shipment) { Aypex::Shipment.new(cost: 10) }
    let(:payment) { Aypex::Payment.new(amount: 10) }
  ...
  describe "#collect_admin_payment_methods" do
    ...
    it { expect(order.collect_admin_payment_methods).to include(credit_card_payment_method) }
    it { expect(order.collect_admin_payment_methods).not_to include(store_credit_payment_method) }
  end
  ...
  describe "#create_shipment_tax_charge!" do
    ...
    context "when order has shipments" do
      before do
        ...
        allow(Aypex::TaxRate).to receive(:adjust).with(order, order_shipments)
      end

      it { expect(order_shipments).to receive(:any?).and_return(true) }
      it { expect(order).to receive(:shipments).and_return(order_shipments) }
      it { expect(Aypex::TaxRate).to receive(:adjust).with(order, order_shipments) }
    end
  ...
  describe "#shipping_eq_billing_address" do
    ...
    context "blank addresses" do
      before do
        order.bill_address = Aypex::Address.new
        order.ship_address = Aypex::Address.new
      end
  ...
  describe "#destroying order will trigger ship and bill addresses destroy" do
    ...
    it { expect { order.destroy }.to change { Aypex::Address.count }.by(-2) }
  end
  ...
  describe "#valid_promotions" do
    ...
    let(:order_promo) { create(:promotion_with_item_adjustment, weighted_order_adjustment_amount: 10, starts_at: Time.now, code: "Order1", id: 2) }
    ...
    let(:source) { Aypex::Promotion::Actions::CreateItemAdjustments.create(calculator: calculator, promotion_id: order_promo.id) }
    let(:zero_calculator) { Aypex::Calculator::Promotion::FlatRate.new(amount: 0) }
    let(:zero_source) { Aypex::Promotion::Actions::CreateItemAdjustments.create calculator: zero_calculator, promotion_id: zero_promo.id }
    let(:line_item_source) { Aypex::Promotion::Actions::CreateItemAdjustments.create calculator: calculator, promotion_id: line_item_promo.id }
    ...
    context "with promotions" do
      ...
      before do
        promotions = [line_item_promo]
        promotions.each do |promotion|
          promotion.orders << order
          promotion.actions << Aypex::Promotion::Actions::CreateItemAdjustments.new
          promotion.rules << Aypex::Promotion::Rules::FirstOrder.new
          promotion.save!
        end
