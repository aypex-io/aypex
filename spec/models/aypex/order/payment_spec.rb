...
module Aypex
  describe Aypex::Order, type: :model do
    ...
    context "ensure source attributes stick around" do
      # For the reason of this test, please see aypex/aypex_gateway#132
      it "does not have inverse_of defined" do
        expect(Aypex::Order.reflections["payments"].options[:inverse_of]).to be_nil
      end
    ...
    describe "#process_payments!" do
      ...
      # Regression spec for https://github.com/aypex/aypex/issues/5436
      ...
      context "when a payment raises a GatewayError" do
        ...
        it "returns true when configured to allow checkout on gateway failures" do
          allow(Aypex::Config).to receive(:allow_checkout_on_gateway_error).and_return(true)
          expect(order.process_payments!).to be true
        end

        it "returns false when not configured to allow checkout on gateway failures" do
          allow(Aypex::Config).to receive(:allow_checkout_on_gateway_error).and_return(false)
    ...
    describe "#outstanding_balance" do
      ...
      it "does not incorporate refunds without a reimbursement" do
        ...
        calculator.amount = order.shipments.first.cost
        calculator.save!
