...
describe Aypex::Order do
  ...
  context "with default state machine" do
    ...
    it "has the following transitions" do
      transitions.each do |transition|
        transition = Aypex::Order.find_transition(from: transition.keys.first, to: transition.values.first)
        expect(transition).not_to be_nil
      end
    end

    it "does not have a transition from delivery to confirm" do
      transition = Aypex::Order.find_transition(from: :delivery, to: :confirm)
      expect(transition).to be_nil
    end

    it ".find_transition when contract was broken" do
      expect(Aypex::Order.find_transition(foo: :bar, baz: :dog)).to be_falsey
    end

    it ".remove_transition" do
      options = {from: transitions.first.keys.first, to: transitions.first.values.first}
      expect(Aypex::Order).to receive_messages(
        removed_transitions: [],
        next_event_transitions: transitions.dup
      )
      expect(Aypex::Order.remove_transition(options)).to be_truthy
      expect(Aypex::Order.removed_transitions).to eql([options])
      expect(Aypex::Order.next_event_transitions).not_to include(transitions.first)
    end

    it ".remove_transition when contract was broken" do
      expect(Aypex::Order.remove_transition(nil)).to be_falsey
    end
    ...
    context "from address" do
      ...
      it "updates totals" do
        ...
        allow(Aypex::TaxRate).to receive_messages match: [tax_rate]
    ...
    context "to delivery" do
      context "when order has default selected_shipping_rate_id" do
        ...
        let(:shipping_rate) do
          [
            Aypex::ShippingRate.create!(shipping_method: shipping_method, cost: 10.00, shipment: shipment)
          ]
        end
  ...
  context "to complete" do
    ...
    context "default credit card" do
      ...
      before do
        ...
        Aypex::OrderUpdater.new(order).update
  ...
  context "re-define checkout flow" do
    ...
    it "does not keep old event transitions when checkout_flow is redefined" do
      expect(Aypex::Order.next_event_transitions).to eq([{cart: :payment}, {payment: :complete}])
    end

    it "does not keep old events when checkout_flow is redefined" do
      state_machine = Aypex::Order.state_machine
  ...
  describe "update_from_params" do
    ...
    context "passing a credit card" do
      let(:permitted_params) do
        Aypex::PermittedAttributes.checkout_attributes +
          [payments_attributes: Aypex::PermittedAttributes.payment_attributes]
      end
      ...
      it "sets confirmation value when its available via :cvc_confirm" do
        allow(Aypex::CreditCard).to receive_messages find: credit_card
      ...
      it "sets existing card as source for new payment" do
        expect do
          order.update_from_params(params, permitted_params)
        end.to change { Aypex::Payment.count }.by(1)

        expect(Aypex::Payment.last.source).to eq credit_card
      end
