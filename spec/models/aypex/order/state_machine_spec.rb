...
describe Aypex::Order, type: :model do
  ...
  before do
    ...
    Aypex::Order.define_state_machine!
  ...
  context "#next!" do
    ...
    context "when current state is delivery" do
      ...
      it "adjusts tax rates when transitioning to delivery" do
        ...
        expect(Aypex::TaxRate).to receive(:adjust).once
      ...
      it "adjusts tax rates twice if there are any shipments" do
        ...
        expect(Aypex::TaxRate).to receive(:adjust).twice
        allow(order).to receive :set_shipments_cost
        order.next!
      end
    end
  ...
  context "#can_cancel?" do
    ...
    (Aypex::Shipment.state_machine.states.keys - [:pending, :backorder, :ready]).each do |shipment_state|
