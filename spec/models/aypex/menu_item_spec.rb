...
describe Aypex::MenuItem, type: :model do
  ...
  describe "#reset_link_attributes from URL to Homepage" do
    let(:i_b) do
      create(:menu_item, name: "Home", item_type: "Link", menu: menu,
        parent: menu.root, linked_resource_type: "Aypex::Linkable::Uri", destination: "http://somewhere.com", new_window: true, linked_resource_id: 5)
    end

    before do
      i_b.update!(linked_resource_type: "Aypex::Linkable::Homepage")
    end
  ...
  describe "#reset_link_attributes from Link to Container" do
    ...
    it "sets resets the destination to nil" do
      expect(i_c.destination).to be_nil
    end
  end
  ...
  describe "#link" do
    ...
    let(:item_url) { create(:menu_item, name: "URL To Random Site", item_type: "Link", menu: menu, linked_resource_type: "Aypex::Linkable::Uri", destination: "https://some-other-website.com") }
    ...
    it "returns nil for destination when product is removed" do
      ...
      expect(item_product.link).to be_nil
    end
    ...
    it "returns nil when URL is nil" do
      expect(item_empty_url.link).to be_nil
    end
  end

  describe "#paremeterize_code" do
    let(:item) { create(:menu_item, name: "URL", item_type: "Link", menu: menu, parent: menu.root, linked_resource_type: "Aypex::Linkable::Uri", code: "My Fantastic Code") }
  ...
  describe "#ensure_item_belongs_to_root" do
    let(:item_x) { create(:menu_item, name: "URL", item_type: "Link", menu: menu, linked_resource_type: "Aypex::Linkable::Uri", code: "My Fantastic Code") }
