...
describe Aypex::TaxCategory, type: :model do
  ...
  describe "#destroy" do
    ...
    it "removes all tax rates" do
      expect { tax_category.destroy }.to change { Aypex::TaxRate.count }.by(-1)
    end

    it "nullifies all products and variants" do
      expect { tax_category.destroy }.not_to change { Aypex::Product.count }
      expect { tax_category.destroy }.not_to change { Aypex::Variant.count }
