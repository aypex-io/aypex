...
RSpec.describe Aypex::Calculator::Shipping::DigitalDelivery do
  subject { Aypex::Calculator::Shipping::DigitalDelivery.new }

  it "has a description for the class" do
    expect(Aypex::Calculator::Shipping::DigitalDelivery).to respond_to(:description)
  end
  ...
  describe "#compute_package" do
    ...
    it "always returns the amount" do
      amount_double = double
      expect(subject).to receive(:amount).and_return(amount_double)
  ...
  describe "#available?" do
    let(:digital_order) do
      ...
      package = Aypex::Stock::Package.new(create(:stock_location), [])
      variants.each do |v|
  ...
  def add_line_item_to_order(order, variant, quantity)
    Aypex::Cart::AddItem.call(order: order, variant: variant, quantity: quantity)
  end
end
