...
describe Aypex::Calculator::Tax::Default, type: :model do
  ...
  let!(:calculator) { Aypex::Calculator::Tax::Default.new(calculable: rate) }
      ...
      context "when the variant matches the tax category" do
        context "when line item is discounted" do
          before do
            ...
            Aypex::TaxRate.store_pre_tax_amount(line_item, [rate])
          end
  ...
  context "when given a line_item" do
    subject do
      Aypex::Calculator::Tax::Default.new(calculable: rate)
