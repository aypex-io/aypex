...
describe Aypex::Calculator::Returns::DefaultRefundAmount, type: :model do
  ...
  let(:calculator) { Aypex::Calculator::Returns::DefaultRefundAmount.new }
  ...
  context "not an exchange" do
    ...
    context "order adjustments" do
      ...
      before do
        order.adjustments << create(:adjustment, order: order, amount: adjustment_amount, eligible: true, label: "Adjustment", source_type: "Aypex::Order")
    ...
    context "shipping adjustments" do
      ...
      before { order.shipments << Aypex::Shipment.new(adjustment_total: adjustment_total) }
