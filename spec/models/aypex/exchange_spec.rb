  ...
  describe Exchange do
    ...
    describe "#display_amount" do
      it "is the total amount of all return items" do
        expect(exchange.display_amount).to eq Aypex::Money.new(0.0)
      end
    end
    ...
    describe "#perform!" do
      ...
      context "when it cannot create shipments for all items" do
        ...
        it "raises an UnableToCreateShipments error" do
          expect do
            subject
          end.to raise_error(Aypex::Exchange::UnableToCreateShipments)
        end
      end
    end
    ...
    describe ".param_key" do # for dom_id
      it { expect(Exchange.param_key).to eq "aypex_exchange" }
    end

    describe ".model_name" do # for dom_id
      it { expect(Exchange.model_name).to eq Aypex::Exchange }
    end
  end
end
