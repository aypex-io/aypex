...
describe Aypex::Variant, type: :model do
  ...
  context "after create" do
    ...
    it "propagate to stock items" do
      expect_any_instance_of(Aypex::StockLocation).to receive(:propagate_variant)
      create(:variant, product: product)
    end

    context "stock location has disable propagate all variants" do
      before { Aypex::StockLocation.update_all propagate_all_variants: false }

      it "propagate to stock items" do
        expect_any_instance_of(Aypex::StockLocation).not_to receive(:propagate_variant)
  ...
  describe "scope" do
    describe ".eligible" do
      context "when only master variants" do
        ...
        it "returns all of them" do
          expect(Aypex::Variant.eligible).to include(product_1.master)
          expect(Aypex::Variant.eligible).to include(product_2.master)
        end
      end
      ...
      context "when product has more than 1 variant" do
        ...
        it "filters master variant out" do
          expect(Aypex::Variant.eligible).to include(variant)
          expect(Aypex::Variant.eligible).not_to include(product.master)
        end
      end
    end
    ...
    describe ".not_discontinued" do
      context "when discontinued" do
        ...
        it { expect(Aypex::Variant.not_discontinued).not_to include(discontinued_variant) }
      end

      context "when not discontinued" do
        let!(:variant_2) { create(:variant, discontinue_on: Time.current + 1.day) }

        it { expect(Aypex::Variant.not_discontinued).to include(variant_2) }
      end
    ...
    describe ".not_deleted" do
      context "when deleted" do
        ...
        it { expect(Aypex::Variant.not_deleted).not_to include(deleted_variant) }
      end

      context "when not deleted" do
        let!(:variant_2) { create(:variant, deleted_at: nil) }

        it { expect(Aypex::Variant.not_deleted).to include(variant_2) }
      end
    end
    ...
    describe ".for_currency_and_available_price_amount" do
      ...
      context "when price with currency present" do
        context "when price has amount" do
          ...
          it { expect(Aypex::Variant.for_currency_and_available_price_amount(currency)).to include(variant) }
        end

        context "when price do not have amount" do
          let!(:price_1) { create(:price, currency: currency, variant: variant, amount: nil) }

          it { expect(Aypex::Variant.for_currency_and_available_price_amount(currency)).not_to include(variant) }
        end
      end
      ...
      context "when currency parameter is nil" do
        ...
        before { Aypex::Store.default.default_currency = currency }

        it { expect(Aypex::Variant.for_currency_and_available_price_amount).to include(variant) }
      end
    end
    ...
    describe ".active" do
      ...
      before do
        allow(Aypex::Variant).to receive(:not_discontinued).and_return(variants)
      ...
      it "finds not_discontinued variants" do
        expect(Aypex::Variant).to receive(:not_discontinued).and_return(variants)
        Aypex::Variant.active(currency)
      end
      ...
      it { expect(Aypex::Variant.active(currency)).to eq(variants) }
    end
  end
  ...
  describe "#cost_price=" do
    it "uses LocalizedNumber.parse" do
      expect(Aypex::LocalizedNumber).to receive(:parse).with("1,599.99")
  ...
  describe "#in_stock_or_backorderable?" do
    ...
    context "when variant has no stock items" do
      before { Aypex::StockItem.delete_all }
  ...
  describe "#options_text" do
    ...
    before do
      allow(Aypex::Variants::OptionsPresenter).to receive(:new).with(variant).and_return(fake_presenter)
    end

    it "calls Aypex::Variants::OptionsPresenter" do
  ...
  describe "#in_stock?" do
    context "when stock_items are not backorderable" do
      before do
        allow_any_instance_of(Aypex::StockItem).to receive_messages(backorderable: false)
      end
      ...
      context "when stock_items out of stock" do
        before do
          allow_any_instance_of(Aypex::StockItem).to receive_messages(backorderable: false)
          allow_any_instance_of(Aypex::StockItem).to receive_messages(count_on_hand: 0)
        end
    ...
    describe "#can_supply?" do
      it "calls out to quantifier" do
        expect(Aypex::Stock::Quantifier).to receive(:new).and_return(quantifier = double)
    ...
    context "when stock_items are backorderable" do
      before do
        allow_any_instance_of(Aypex::StockItem).to receive_messages(backorderable: true)
      end
  ...
  describe "#is_backorderable" do
    ...
    it "invokes Aypex::Stock::Quantifier" do
      expect_any_instance_of(Aypex::Stock::Quantifier).to receive(:backorderable?).and_return(true)
      subject
    end
  end
  ...
  describe "#total_on_hand" do
    ...
    it "matches quantifier total_on_hand" do
      ...
      expect(variant.total_on_hand).to eq(Aypex::Stock::Quantifier.new(variant).total_on_hand)
    end
  end
  ...
  describe "in_stock scope" do
    it "returns all in stock variants" do
      ...
      expect(Aypex::Variant.in_stock).to eq [in_stock_variant]
    end
  end
  ...
  describe "#check_price" do
    ...
    context "require_master_price set false" do
      ...
      context "price present and currency present" do
        it { expect(variant.send(:check_price)).to be_nil }
      end

      context "price present and currency nil" do
        before { variant.currency = nil }

        it { expect(variant.send(:check_price)).to eq(Aypex::Store.default.default_currency) }
      end
    ...
    context "require_master_price set true" do
      ...
      context "product and master_variant present and equal" do
        ...
        context "price nil and currency nil" do
          ...
          it { expect(variant.price).to eq(variant.product.master.price) }
          it { expect(variant.currency).to eq(Aypex::Store.default.default_currency) }
        end
      end
        ...
        context "product not present" do
          ...
          context "price nil and currency present" do
            ...
            it "adds absence of master error" do
              variant.send(:check_price)
              expect(variant.errors[:base]).to include I18n.t("activerecord.errors.models.aypex/variant.attributes.base.no_master_variant_found_to_infer_price")
            end
          end
  ...
  describe "#ensure_not_in_complete_orders" do
    ...
    it "adds error on variant destroy" do
      ...
      expect(variant.errors[:base]).to include I18n.t("activerecord.errors.models.aypex/variant.attributes.base.cannot_destroy_if_attached_to_line_items")
    end
  end
  ...
  describe "#remove_line_items_from_incomplete_orders" do
    ...
    it "schedules RemoveFromIncompleteOrdersJob" do
      expect(Aypex::Variants::RemoveFromIncompleteOrdersJob).to receive(:perform_later).with(variant)
      variant.destroy
    end
