...
describe Aypex::Product, type: :model do
  ...
  context "product instance" do
    ...
    describe "#duplicate" do
      ...
      it "calls #duplicate_extra" do
        expect_any_instance_of(Aypex::Product).to receive(:duplicate_extra)
          .with(product)
    ...
    describe "#display_price" do
      ...
      context "with currency set to JPY" do
        before do
          ...
          store.default_currency = "JPY"
        end

        after { store.default_currency = "USD" }
    ...
    describe "#discontinue_on_must_be_later_than_make_active_at" do
      ...
      context "make_active_at is a date earlier than discontinue_on" do
        ...
        context "is not valid" do
          ...
          it { expect(product.errors[:discontinue_on]).to include(I18n.t(:invalid_date_range, scope: "activerecord.errors.models.aypex/product.attributes.discontinue_on")) }
        end
      end
  ...
  context "properties" do
    ...
    context "optional property_presentation" do
      subject { Aypex::Property.where(name: "foo").first.presentation }
    ...
    it "does not overwrite properties' presentation names" do
      Aypex::Property.where(name: "foo").first_or_create!(presentation: "Foo's Presentation Name")
      product.set_property("foo", "value1")
      product.set_property("bar", "value2")
      expect(Aypex::Property.where(name: "foo").first.presentation).to eq("Foo's Presentation Name")
      expect(Aypex::Property.where(name: "bar").first.presentation).to eq("bar")
    end
  ...
  describe "#images" do
    ...
    let(:params) { {viewable_id: product.master.id, viewable_type: "Aypex::Variant", alt: "position 2", position: 2} }
  ...
  context "classifications and taxons" do
    ...
    it "will delete all classifications" do
      reflection = Aypex::Product.reflect_on_association(:classifications)
  ...
  # Regression spec for https://github.com/aypex/aypex/issues/5588
  ...
  describe "#ensure_not_in_complete_orders" do
    ...
    it "adds error on product destroy" do
      ...
      expect(product.errors[:base]).to include I18n.t("activerecord.errors.models.aypex/product.attributes.base.cannot_destroy_if_attached_to_line_items")
    end
  end
  ...
  describe "#default_variant_cache_key" do
    ...
    context "with inventory tracking" do
      it "returns proper key" do
        expect(key).to eq("aypex/default-variant/#{product.cache_key_with_version}/true")
      end
    end
    ...
    context "with more variants aside from the master variant" do
      ...
      context "with at least one non-master variant stock items count_on_hand > 0" do
        before do
          ...
          Aypex::StockItem.all.each { |stock_item| stock_item.set_count_on_hand(1) }
        end
