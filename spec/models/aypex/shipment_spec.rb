...
describe Aypex::Shipment do
  ...
  describe "precision of pre_tax_amount" do
    ...
    it "keeps four digits of precision even when reloading" do
      ...
      allow_any_instance_of(Aypex::LineItem).to receive(:update_tax_charge)
  ...
  context "shipping_rates" do
    ...
    let(:shipping_rates) do
      [
        Aypex::ShippingRate.new(shipping_method: shipping_method1, cost: 10.00, selected: true),
        Aypex::ShippingRate.new(shipping_method: shipping_method2, cost: 20.00)
      ]
    end
    ...
    context "refresh_rates" do
      ...
      it "requests new rates, and maintain shipping_method selection" do
        expect(Aypex::Stock::Estimator).to receive(:new).with(shipment.order).and_return(mock_estimator)
      ...
      it "does not refresh if shipment is shipped" do
        expect(Aypex::Stock::Estimator).not_to receive(:new)
        shipment.shipping_rates.delete_all
  ...
  describe "#update!" do
    ...
    context "when shipment state changes to shipped" do
      before do
        allow_any_instance_of(Aypex::ShipmentHandler).to receive(:send_shipped_email)
      ...
      context "when using the default shipment handler" do
        it "calls the 'perform' method" do
          shipment.state = "pending"
          allow(shipment).to receive_messages determine_state: "shipped"
          expect_any_instance_of(Aypex::ShipmentHandler).to receive(:perform)
          shipment.update!(order)
        end
      end
      ...
      context "when using a custom shipment handler" do
        before do
          Aypex::ShipmentHandler::UPS = Class.new do
        ...
        after do
          Aypex::ShipmentHandler.send(:remove_const, :UPS)
        end

        it "calls the custom handler's 'perform' method" do
          shipment.state = "pending"
          allow(shipment).to receive_messages determine_state: "shipped"
          expect_any_instance_of(Aypex::ShipmentHandler::UPS).to receive(:perform)
          shipment.update!(order)
        end
      end
      ...
      context "with adjustments" do
        before do
          shipment.adjustments << Aypex::Adjustment.create(order: order, label: "Label", amount: 5)
        end
  ...
  describe "#cancel" do
    ...
    context "with backordered inventory units" do
      ...
      before do
        Aypex::Cart::AddItem.call(order: order, variant: variant)
        order.create_proposed_shipments

        Aypex::Cart::AddItem.call(order: other_order, variant: variant)
    ...
    context "with Config.auto_capture_on_dispatch == true" do
      ...
      context "order has pending payments" do
        ...
        before do
          ...
          calculator.amount = @shipment.cost
          calculator.save!
        end
  ...
  context "changes shipping rate via general update" do
    ...
    let(:shipment) { Aypex::Shipment.create order_id: order.id, stock_location: create(:stock_location) }

    let(:shipping_rate) do
      Aypex::ShippingRate.create shipment_id: shipment.id, cost: 10
    end
  ...
  describe "#transfer_to_location" do
    ...
    before do
      ...
      shipping_method.calculator.amount = order.shipments.first.cost
  ...
  describe "#destroy" do
    it "destroys linked shipping_rates" do
      reflection = Aypex::Shipment.reflect_on_association(:shipping_rates)
