...
describe Aypex::Search::Base do
  ...
  it "accepts multiple currencies" do
    ...
    Aypex::Store.default.update(default_currency: "PLN")
  ...
  describe "searching by a current store" do
    ...
    let(:aypex_store) { create(:store, name: "Aypex Demo Site", default_currency: "USD", default: true) }
    ...
    let!(:product_for_aypex_store) { create(:product, name: "P-Aypex-1", stores: [aypex_store]) }
    ...
    context "current sture is given in params" do
      ...
      context "when Aypex Store" do
        let(:current_store) { aypex_store }

        it "returns 1 product" do
          expect(retrieved_products.count).to eq(1)
        end

        it "returns only products from Aypex store" do
          expect(retrieved_products).to contain_exactly(product_for_aypex_store)
        end
      end
    end
  end
