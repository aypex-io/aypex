...
describe Aypex::ServiceModule do
  context "non-callable thing passed to run" do
    ...
    it "raises NonCallablePassedToRun" do
      expect { result }.to raise_error(Aypex::ServiceModule::NonCallablePassedToRun)
    end
  end
  ...
  context "unimplemented method" do
    ...
    it "raises MethodNotImplemented" do
      expect { result }.to raise_error(Aypex::ServiceModule::MethodNotImplemented)
    end

    it "returns message in exception" do
      result
    rescue Aypex::ServiceModule::MethodNotImplemented => e
  ...
  context "non wrapped value" do
    ...
    it "raises WrongDataPassed" do
      expect { ServiceObjectWithNonWrappedReturn.new.call }.to raise_error(Aypex::ServiceModule::WrongDataPassed)
    end

    it "returns message in exception" do
      ServiceObjectWithNonWrappedReturn.new.call
    rescue Aypex::ServiceModule::WrongDataPassed => e
  ...
  context "first method failed" do
    ...
    it "returns result which is instance of Result" do
      expect(result).to be_an_instance_of(Aypex::ServiceModule::Result)
    end
  ...
  context "not compatible params passed as result" do
    ...
    it "raises exception" do
      expect { service.call }.to raise_error(Aypex::ServiceModule::IncompatibleParamsPassed)
    end
  end
end
