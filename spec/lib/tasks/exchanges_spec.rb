...
describe "exchanges:charge_unreturned_items" do
  ...
  context "there are no unreturned items" do
    it { expect { subject.invoke }.not_to change { Aypex::Order.count } }
  end
  ...
  context "there are unreturned items" do
    ...
    context "fewer than the config allowed days have passed" do
      let(:travel_time) { (Aypex::Config.expedited_exchanges_days_window - 1).days }

      it "does not create a new order" do
        expect { subject.invoke }.not_to change { Aypex::Order.count }
      end
    end
    ...
    context "more than the config allowed days have passed" do
      ...
      it "creates a new completed order" do
        expect { subject.invoke }.to change { Aypex::Order.count }
        expect(Aypex::Order.last).to be_completed
      end

      it "moves the shipment for the unreturned items to the new order" do
        subject.invoke
        new_order = Aypex::Order.last
        expect(new_order.shipments.count).to eq 1
        expect(return_item_2.reload.exchange_shipments.first.order).to eq Aypex::Order.last
      end

      it "creates line items on the order for the unreturned items" do
        subject.invoke
        expect(Aypex::Order.last.line_items.map(&:variant)).to eq [return_item_2.exchange_variant]
      end

      it "associates the exchanges inventory units with the new line items" do
        subject.invoke
        expect(return_item_2.reload.exchange_inventory_units.last.try(:line_item).try(:order)).to eq Aypex::Order.last
      end
      ...
      it "authorizes the order for the full amount of the unreturned items including taxes" do
        expect { subject.invoke }.to change { Aypex::Payment.count }.by(1)
        new_order = Aypex::Order.last
      ...
      it "associates the store of the original order with the exchange order" do
        allow_any_instance_of(Aypex::Order).to receive(:store_id).and_return(123)

        expect(Aypex::Order).to receive(:create!).once.with(hash_including(store_id: 123)) { |attrs| Aypex::Order.new(attrs.except(:store_id)).tap(&:save!) }
        subject.invoke
      end
      ...
      context "there is no card from the previous order" do
        ...
        before { allow_any_instance_of(Aypex::Order).to receive(:valid_credit_cards).and_return([]) }
      ...
      context "it is unable to authorize the credit card" do
        before { allow_any_instance_of(Aypex::Payment).to receive(:authorize!).and_raise(RuntimeError) }
